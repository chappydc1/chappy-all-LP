import { PipiteaMatchaPromiseItem } from "./components/PipiteaMatchaPromiseItem";

export const PipiteaMatchaProductPromise = () => {
  return (
    <section
      href=""
      className="items-center bg-no-repeat box-border caret-transparent flex flex-col justify-items-center min-h-[auto] min-w-[auto] outline-[3px] w-full border-emerald-800/20 bg-center px-[15px] py-[45px] border-b border-t md:py-[60px]"
    >
      <div
        href=""
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1170px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center md:flex-row"
      >
        <div
          href=""
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="text-black text-[27.0583px] box-border caret-transparent leading-[32.47px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-[35px] md:leading-[42px]"
            >
              <div
                href=""
                className="text-[27.0583px] box-border caret-transparent leading-[32.47px] outline-[3px] w-full md:text-[35px] md:leading-[42px]"
              >
                <p
                  href=""
                  className="text-[27.0583px] box-border caret-transparent leading-[32.47px] outline-[3px] md:text-[35px] md:leading-[42px]"
                >
                  <strong
                    href=""
                    className="text-[27.0583px] font-bold box-border caret-transparent leading-[32.47px] outline-[3px] md:text-[35px] md:leading-[42px]"
                  >
                    PiPi Tea Ceremonial Matcha Promise
                  </strong>
                </p>
              </div>
            </div>
          </div>
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <div
                href=""
                className="items-start box-border caret-transparent hidden flex-wrap mt-[30px] md:mt-10"
              >
                <PipiteaMatchaPromiseItem
                  outerClassName="box-border caret-transparent flex basis-[0%] flex-col grow outline-[3px]"
                  wrapperClassName="items-center box-border caret-transparent flex justify-center outline-[3px]"
                  contentClassName="box-border caret-transparent flex flex-col outline-[3px] w-full"
                  containerClassName="items-start box-border caret-transparent flex flex-wrap"
                  itemOuterClassName="box-border caret-transparent flex basis-[0%] flex-col grow outline-[3px]"
                  itemWrapperClassName="items-center box-border caret-transparent flex justify-center outline-[3px]"
                  itemContentClassName="box-border caret-transparent flex flex-col outline-[3px] w-full"
                  itemContainerClassName="items-start box-border caret-transparent flex flex-wrap"
                  iconWrapperClassName="items-center box-border caret-transparent flex flex-col outline-[3px] w-full"
                  imageClassName="box-border caret-transparent brightness-0 saturate-100 invert-[0.18] sepia-[0.2] saturate-[32] hue-rotate-[250deg] brightness-90 contrast-[0.95] max-w-full outline-[3px] w-[94px]"
                  imageSizes=""
                  firstImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/45.png"
                  firstText="Ceremonial Grade"
                  firstTextUsesSpan="true"
                  dividerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-w-2.5 outline-[3px] w-2.5"
                  secondImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/46.png"
                  secondText="First Harvest"
                  secondImageSizes=""
                  thirdImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/47.png"
                  thirdText="Pesticide-Free "
                  thirdImageSizes=""
                  showThirdItem="true"
                />
                <div
                  href=""
                  className="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-w-2.5 outline-[3px] w-2.5"
                >
                  <div
                    href=""
                    className="items-center box-border caret-transparent flex justify-center outline-[3px]"
                  >
                    <div
                      href=""
                      className="box-border caret-transparent flex flex-col outline-[3px] w-full"
                    >
                      <div
                        href=""
                        className="items-start box-border caret-transparent flex flex-wrap mt-[30px]"
                      >
                        <div
                          href=""
                          className="box-border caret-transparent flex basis-[0%] flex-col grow outline-[3px]"
                        ></div>
                      </div>
                    </div>
                  </div>
                </div>
                <PipiteaMatchaPromiseItem
                  outerClassName="box-border caret-transparent flex basis-[0%] flex-col grow outline-[3px]"
                  wrapperClassName="items-center box-border caret-transparent flex justify-center outline-[3px]"
                  contentClassName="box-border caret-transparent flex flex-col outline-[3px] w-full"
                  containerClassName="items-start box-border caret-transparent flex flex-wrap"
                  itemOuterClassName="box-border caret-transparent flex basis-[0%] flex-col grow outline-[3px]"
                  itemWrapperClassName="items-center box-border caret-transparent flex justify-center outline-[3px]"
                  itemContentClassName="box-border caret-transparent flex flex-col outline-[3px] w-full"
                  itemContainerClassName="items-start box-border caret-transparent flex flex-wrap"
                  iconWrapperClassName="items-center box-border caret-transparent flex flex-col outline-[3px] w-full"
                  imageClassName="box-border caret-transparent brightness-0 saturate-100 invert-[0.18] sepia-[0.2] saturate-[32] hue-rotate-[250deg] brightness-90 contrast-[0.95] max-w-full outline-[3px] w-[94px]"
                  imageSizes=""
                  firstImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/1762824038752_certification03.png"
                  firstText="USDA Organic"
                  firstTextUsesSpan="true"
                  dividerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-w-2.5 outline-[3px] w-2.5"
                  secondImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/49.png"
                  secondText="Triple Toxin Tested"
                  secondImageSizes=""
                  thirdImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/52.png"
                  thirdText="Manufactured in an FDA-Registered Facility"
                  thirdImageSizes=""
                  showThirdItem="true"
                />
              </div>
            </div>
          </div>
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <div
                href=""
                className="items-start box-border caret-transparent flex flex-col flex-wrap min-h-[auto] min-w-[auto] mt-[30px] md:flex-row md:mt-10"
              >
                <PipiteaMatchaPromiseItem
                  outerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-full min-h-[auto] min-w-full outline-[3px] w-full md:max-w-none md:min-w-0 md:w-auto"
                  wrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  contentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  containerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  itemOuterClassName="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                  itemWrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  itemContentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  itemContainerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  iconWrapperClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  imageClassName="box-border caret-transparent brightness-0 saturate-100 invert-[0.18] sepia-[0.2] saturate-[32] hue-rotate-[250deg] brightness-90 contrast-[0.95] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[94px]"
                  imageSizes="94px"
                  firstImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/49.png"
                  firstText="Triple Toxin Tested"
                  firstTextUsesSpan="false"
                  dividerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-h-[auto] min-w-2.5 outline-[3px] w-2.5"
                  secondImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/46.png"
                  secondText="First Harvest"
                  secondImageSizes="94px"
                  thirdImageSrc=""
                  thirdText=""
                  thirdImageSizes=""
                  showThirdItem="false"
                />
                <div
                  href=""
                  className="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-h-[auto] min-w-2.5 outline-[3px] w-2.5"
                >
                  <div
                    href=""
                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  >
                    <div
                      href=""
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <div
                        href=""
                        className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto] mt-[25px]"
                      >
                        <div
                          href=""
                          className="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                        ></div>
                      </div>
                    </div>
                  </div>
                </div>
                <PipiteaMatchaPromiseItem
                  outerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-full min-h-[auto] min-w-full outline-[3px] w-full md:max-w-none md:min-w-0 md:w-auto"
                  wrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  contentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  containerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  itemOuterClassName="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                  itemWrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  itemContentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  itemContainerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  iconWrapperClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  imageClassName="box-border caret-transparent brightness-0 saturate-100 invert-[0.18] sepia-[0.2] saturate-[32] hue-rotate-[250deg] brightness-90 contrast-[0.95] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[94px]"
                  imageSizes=""
                  firstImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/47.png"
                  firstText="Pesticide Free"
                  firstTextUsesSpan="false"
                  dividerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-h-[auto] min-w-2.5 outline-[3px] w-2.5"
                  secondImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/1762824038752_certification03.png"
                  secondText="USDA Organic"
                  secondImageSizes="94px"
                  thirdImageSrc=""
                  thirdText=""
                  thirdImageSizes=""
                  showThirdItem="false"
                />
                <div
                  href=""
                  className="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-h-[auto] min-w-2.5 outline-[3px] w-2.5"
                >
                  <div
                    href=""
                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  >
                    <div
                      href=""
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <div
                        href=""
                        className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto] mt-[25px]"
                      >
                        <div
                          href=""
                          className="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                        ></div>
                      </div>
                    </div>
                  </div>
                </div>
                <PipiteaMatchaPromiseItem
                  outerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-full min-h-[auto] min-w-full outline-[3px] w-full md:max-w-none md:min-w-0 md:w-auto"
                  wrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  contentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  containerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  itemOuterClassName="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                  itemWrapperClassName="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  itemContentClassName="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  itemContainerClassName="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                  iconWrapperClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                  imageClassName="box-border caret-transparent brightness-0 saturate-100 invert-[0.18] sepia-[0.2] saturate-[32] hue-rotate-[250deg] brightness-90 contrast-[0.95] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[94px]"
                  imageSizes="94px"
                  firstImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/45.png"
                  firstText="Ceremonial Grade"
                  firstTextUsesSpan="false"
                  dividerClassName="box-border caret-transparent flex basis-[0%] flex-col grow max-w-2.5 min-h-[auto] min-w-2.5 outline-[3px] w-2.5"
                  secondImageSrc="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/52.png"
                  secondText="Manufactured in an FDA-Registered Facility"
                  secondImageSizes="94px"
                  thirdImageSrc=""
                  thirdText=""
                  thirdImageSizes=""
                  showThirdItem="false"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
