import { PipiteaMatchaTextImageBlock } from "./components/TextImageBlock";

export const PipiteaMatchaProblemSolutionSection = () => {
  return (
    <section
      href=""
      className="relative items-center bg-zinc-300/30 bg-no-repeat box-border caret-transparent flex flex-col justify-items-center min-h-[auto] min-w-[auto] outline-[3px] w-full bg-center px-[15px] py-[45px] md:py-[60px]"
    >
      <div
        href=""
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1170px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center md:flex-row"
      >
        <div
          href=""
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <PipiteaMatchaTextImageBlock
                layoutVariant="text-first"
                imageUrl="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/1762562842908_section01_matcha_work.webp"
                imageSizes="565px"
                heading="Modern life is destroying your cells, and your body can't keep up with the damage."
                headingWrapperClassName="text-black text-[26.4583px] leading-[31.75px] min-h-[auto] min-w-[auto] text-center md:text-3xl md:leading-9 md:text-left"
                headingInnerWrapperClassName="text-[26.4583px] box-border caret-transparent leading-[31.75px] outline-[3px] text-center w-full md:text-3xl md:leading-9 md:text-left"
                headingClassName="text-[26.4583px] font-medium box-border caret-transparent leading-[31.75px] outline-[3px] text-center md:text-3xl md:leading-9 md:text-left"
                strongClassName="text-[26.4583px] font-bold box-border caret-transparent leading-[31.75px] outline-[3px] text-center md:text-3xl md:leading-9 md:text-left"
                content={
                  <>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            You're not imagining it. The bloating that makes you
                            look 6 months pregnant after any meal. The 15 pounds
                            around your middle that won't budge no matter how
                            &quot;clean&quot; you eat. The joint pain that makes
                            you feel decades older. The brain fog, the
                            exhaustion that sleep doesn't fix.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              These aren't separate problems. They're all
                              symptoms of the same root cause: oxidative stress
                              and cellular damage.
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            Chronic stress, declining hormone levels as we age,
                            environmental toxins, processed foods, and
                            inflammatory triggers create an overwhelming burden
                            of free radicals. These are unstable molecules that
                            damage your gut lining, hormone receptors, fat
                            cells, joints, and brain.
                          </p>
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] mt-5 md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              Your body tries to repair itself. But without
                              enough antioxidants, it can't keep up.
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] mt-px md:text-lg md:leading-[27px]"
                          >
                            The damage accumulates. Your gut lining develops
                            gaps (leaky gut). Your hormone receptors stop
                            responding properly. Your fat cells become inflamed
                            and insulin-resistant. Your joints fill with
                            inflammatory cytokines. Your brain struggles with
                            reduced blood flow and neuronal damage.
                          </p>
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] mt-5 md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              And suddenly, nothing works anymore.
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                  </>
                }
              />
            </div>
          </div>
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <img
                src="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/14.svg"
                alt=""
                sizes="1920px"
                href=""
                title=""
                className="absolute box-border caret-transparent max-w-full outline-[3px] w-full top-0 md:top-[-45px]"
              />
            </div>
          </div>
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <img
                src="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/15.svg"
                alt=""
                sizes="1920px"
                href=""
                title=""
                className="absolute box-border caret-transparent max-w-full outline-[3px] w-full bottom-0 md:bottom-[-45px]"
              />
            </div>
          </div>
        </div>
      </div>
      <div
        href=""
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1170px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center mt-[50px] md:flex-row md:mt-[60px]"
      >
        <div
          href=""
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            href=""
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              href=""
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <PipiteaMatchaTextImageBlock
                layoutVariant="image-first"
                imageUrl="https://c.animaapp.com/mq0c6l4mp0WBgi/assets/1762558489626_section02_matcha_on_cup.webp"
                imageSizes=""
                heading="The Antioxidant-Rich Tea Backed by Centuries of Traditional Use"
                headingWrapperClassName="text-black text-[27.0583px] box-border caret-transparent leading-[32.47px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-3xl md:leading-9 md:text-left"
                headingInnerWrapperClassName="text-[27.0583px] box-border caret-transparent leading-[32.47px] outline-[3px] text-center w-full md:text-3xl md:leading-9 md:text-left"
                headingClassName="text-[27.0583px] font-medium box-border caret-transparent leading-[32.47px] outline-[3px] text-center md:text-3xl md:leading-9 md:text-left"
                strongClassName="text-[27.0583px] font-bold box-border caret-transparent leading-[32.47px] outline-[3px] text-center md:text-3xl md:leading-9 md:text-left"
                content={
                  <>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              Pipi Tea Ceremonial Matcha delivers the highest
                              concentration of EGCG (epigallocatechin gallate),
                              the most powerful antioxidant in green tea, in its
                              most bioavailable form.
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] mt-5 md:text-lg md:leading-[27px]"
                          >
                            Unlike probiotics that can trigger SIBO and worsen
                            bloating, ashwagandha that works inconsistently and
                            carries liver toxicity risks, or restrictive
                            elimination diets that slow your metabolism, matcha
                            provides whole-food antioxidant support that
                            addresses cellular damage at its source.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              With up to 137 times more antioxidants than
                              regular green tea, first-harvest ceremonial matcha
                              represents the most concentrated, pure form of
                              this ancient healing food.
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              Just one cup each morning, prepared in 30 seconds,
                              helps to:
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] pl-5 md:text-lg md:leading-[27px] md:pl-[25px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] items-baseline box-border caret-transparent flex leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px] before:accent-auto before:bg-zinc-700 before:box-border before:caret-transparent before:text-black before:block before:text-[16.0583px] before:not-italic before:normal-nums before:font-normal before:h-2.5 before:tracking-[normal] before:leading-[24.0875px] before:list-outside before:list-disc before:min-h-[auto] before:min-w-2.5 before:outline-[3px] before:pointer-events-auto before:text-left before:no-underline before:indent-[0px] before:normal-case before:translate-y-[-1.4px] before:visible before:w-2.5 before:mr-[15px] before:rounded-[50%] before:border-separate before:font-libre_franklin before:md:text-lg before:md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent block leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] md:text-lg md:leading-[27px]">
                              Repair and seal leaky gut
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-3 pl-5 md:text-lg md:leading-[27px] md:pl-[25px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] items-baseline box-border caret-transparent flex leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px] before:accent-auto before:bg-zinc-700 before:box-border before:caret-transparent before:text-black before:block before:text-[16.0583px] before:not-italic before:normal-nums before:font-normal before:h-2.5 before:tracking-[normal] before:leading-[24.0875px] before:list-outside before:list-disc before:min-h-[auto] before:min-w-2.5 before:outline-[3px] before:pointer-events-auto before:text-left before:no-underline before:indent-[0px] before:normal-case before:translate-y-[-1.4px] before:visible before:w-2.5 before:mr-[15px] before:rounded-[50%] before:border-separate before:font-libre_franklin before:md:text-lg before:md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent block leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] md:text-lg md:leading-[27px]">
                              Support healthy weight loss and metabolism
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full pl-5 md:text-lg md:leading-[27px] md:pl-0"
                      ></div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-3 pl-5 md:text-lg md:leading-[27px] md:pl-[25px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] items-baseline box-border caret-transparent flex leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px] before:accent-auto before:bg-zinc-700 before:box-border before:caret-transparent before:text-black before:block before:text-[16.0583px] before:not-italic before:normal-nums before:font-normal before:h-2.5 before:tracking-[normal] before:leading-[24.0875px] before:list-outside before:list-disc before:min-h-[auto] before:min-w-2.5 before:outline-[3px] before:pointer-events-auto before:text-left before:no-underline before:indent-[0px] before:normal-case before:translate-y-[-1.4px] before:visible before:w-2.5 before:mr-[15px] before:rounded-[50%] before:border-separate before:font-libre_franklin before:md:text-lg before:md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent block leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] md:text-lg md:leading-[27px]">
                              Balance hormones naturally
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-3 pl-5 md:text-lg md:leading-[27px] md:pl-[25px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] items-baseline box-border caret-transparent flex leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px] before:accent-auto before:bg-zinc-700 before:box-border before:caret-transparent before:text-black before:block before:text-[16.0583px] before:not-italic before:normal-nums before:font-normal before:h-2.5 before:tracking-[normal] before:leading-[24.0875px] before:list-outside before:list-disc before:min-h-[auto] before:min-w-2.5 before:outline-[3px] before:pointer-events-auto before:text-left before:no-underline before:indent-[0px] before:normal-case before:translate-y-[-1.4px] before:visible before:w-2.5 before:mr-[15px] before:rounded-[50%] before:border-separate before:font-libre_franklin before:md:text-lg before:md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent block leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] md:text-lg md:leading-[27px]">
                              Reduce chronic inflammation and joint pain
                              <br className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]" />
                            </b>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      href=""
                      className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                    >
                      <div
                        href=""
                        className="text-black text-[16.0583px] box-border caret-transparent leading-[24.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-left w-full mt-[25px] md:text-lg md:leading-[27px]"
                      >
                        <div
                          href=""
                          className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] w-full md:text-lg md:leading-[27px]"
                        >
                          <p
                            href=""
                            className="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]"
                          >
                            <b className="text-[16.0583px] font-bold box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-lg md:leading-[27px]">
                              Feel lighter, think clearer, and move with ease.
                            </b>
                            With 900 years of traditional use and modern
                            clinical validation showing EGCG's powerful effects
                            on gut integrity, metabolism, and inflammation,
                            Ceremonial Matcha helps you address the root cause,
                            not just mask symptoms.
                          </p>
                        </div>
                      </div>
                    </div>
                  </>
                }
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
