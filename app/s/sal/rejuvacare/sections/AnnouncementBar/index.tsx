export const RejuvacareAnnouncementBar = () => {
  return (
    <div className="items-center self-center bg-zinc-800 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
      <div className="relative box-border caret-transparent basis-full grow max-w-full min-h-[25px] outline-[3px] w-min md:basis-0">
        <div className="bg-neutral-100 box-border caret-transparent outline-[3px] w-full">
          <div className="relative items-stretch bg-emerald-500 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] z-10 pt-0 pb-1 px-0 md:flex-nowrap md:px-2.5 md:py-[5px]">
            <div className="relative box-border caret-transparent basis-full grow max-w-full min-h-[25px] outline-[3px] w-min p-px md:basis-0">
              <div className="text-[13px] box-border caret-transparent leading-4 outline-[3px] p-px font-montserrat md:text-base md:leading-6"></div>
              <div className="text-white text-[13px] font-bold box-border caret-transparent basis-[0%] shrink-0 outline-[3px] px-0 py-px rounded-[1px] font-montserrat md:text-base md:px-px">
                🌸 SPRING SALE: : FREE Shipping + 50% OFF Today ONLY!
              </div>
            </div>
          </div>
          <div className="relative items-center bg-stone-100 box-border caret-transparent flex basis-[0%] shrink-0 flex-wrap justify-center max-w-full outline-[3px] z-10 mx-0 pb-0 px-0 rounded-[1px] md:bg-neutral-100 md:flex-nowrap md:mx-px md:pb-px md:px-px">
            <div className="relative items-start bg-stone-100 box-border caret-transparent flex basis-full flex-col-reverse grow max-w-[1400px] min-h-[25px] outline-[3px] w-min py-0 md:items-center md:basis-0 md:flex-row md:py-px">
              <div className="items-start self-center box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full mt-0 pt-2.5 pb-0 px-[15px] md:items-center md:w-[70%] md:mt-2.5 md:pl-[90px] md:pr-[30px] md:pt-px md:pb-3">
                <div className="text-stone-700 text-[40px] font-extrabold box-border caret-transparent leading-[48px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:text-[53px] md:leading-[63.6px]">
                  <div className="text-[40px] box-border caret-transparent leading-[48px] outline-[3px] md:text-[53px] md:leading-[63.6px]">
                    <span className="text-[40px] box-border caret-transparent leading-[48px] outline-[3px] md:text-[53px] md:leading-[63.6px]"></span>
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full"></div>
                <div className="text-[32px] font-bold box-border caret-transparent leading-[48px] min-h-[auto] min-w-[auto] outline-[3px] mt-[15px] font-helvetica"></div>
                <div className="text-[28px] font-bold box-border caret-transparent hidden basis-[0%] shrink-0 leading-9 min-h-0 min-w-0 outline-[3px] text-left mb-[15px] rounded-[1px] font-montserrat md:text-[34px] md:block md:leading-[46px] md:min-h-[auto] md:min-w-[auto]">
                  <span className="text-red-700 text-[28px] box-border caret-transparent leading-9 outline-[3px] md:text-[34px] md:leading-[46px]">
                    Avoid Knee Surgery: 
                  </span>
                  This Simple Device Rebuilds Joints From Home In Just
                  15-Minutes Per Day
                </div>
                <div className="text-[28px] font-bold box-border caret-transparent block basis-[0%] shrink-0 leading-9 min-h-[auto] min-w-[auto] outline-[3px] text-left mb-[15px] rounded-[1px] font-montserrat md:text-[34px] md:hidden md:leading-[46px] md:min-h-0 md:min-w-0">
                  <span className="text-red-700 text-[28px] box-border caret-transparent leading-9 outline-[3px] md:text-[34px] md:leading-[46px]">
                    Avoid Knee Surgery: 
                  </span>
                  This Simple Device Rebuilds Joints From Home In Just
                  15-Minutes Per Day ⏰
                </div>
                <div className="text-stone-700 text-[22px] font-semibold box-border caret-transparent leading-[30.8px] min-h-[auto] min-w-[auto] outline-[3px] text-left mt-0 p-px font-montserrat md:text-[26px] md:leading-[36.4px] md:mt-px">
                  <div className="text-[22px] box-border caret-transparent leading-[30.8px] outline-[3px] md:text-[26px] md:leading-[36.4px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent basis-[0%] shrink-0 leading-[32.2px] outline-[3px] pr-0 rounded-[1px] md:text-[33px] md:leading-[46.2px] md:pr-5"></div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full mb-3 md:items-center">
                  <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-9">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                      alt=""
                      className="text-black box-border caret-transparent h-[26px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[26px] mt-[7px] mb-px md:my-0"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="font-bold box-border caret-transparent outline-[3px] text-left mt-[5px] font-montserrat md:mt-0">
                      Fast Relief From Stiff, Painful, Swollen Knees
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full mb-3 md:items-center">
                  <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-9">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                      alt=""
                      className="text-black box-border caret-transparent h-[26px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[26px] mt-[9px] mb-px md:my-0"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="font-bold box-border caret-transparent outline-[3px] text-left mt-[5px] font-montserrat md:mt-0">
                      Improves Blood Flow And Promotes Healing
                    </div>
                  </div>
                </div>
                <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full mb-3">
                  <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-9">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                      alt=""
                      className="text-black box-border caret-transparent h-[26px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[26px] mt-1.5 mb-px md:my-0"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="text-[17px] font-bold box-border caret-transparent outline-[3px] text-left mt-[5px] font-montserrat md:text-base md:mt-0">
                      <div className="box-border caret-transparent outline-[3px]">
                        Relieves Knee Pain, Bone-on-Bone Arthritis
                      </div>
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full mb-3 md:items-center">
                  <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-9">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                      alt=""
                      className="text-black box-border caret-transparent h-[26px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[26px] mt-1.5 mb-px md:my-0"
                    />
                  </div>
                  <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full pt-px md:[align-items:normal] md:block md:pt-0">
                    <div className="text-[17px] font-bold box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left mt-[5px] font-montserrat md:text-base md:min-h-0 md:min-w-0 md:mt-0">
                      <div className="box-border caret-transparent outline-[3px] mt-px md:mt-0">
                        Extension Straps To Fit All Knee Sizes
                      </div>
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex flex-col justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full mt-0 p-px md:mt-5">
                  <div className="box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full mt-[7px] p-px md:mt-px">
                    <a
                      title="Get up to 50% off today only  "
                      className="text-white text-xl font-extrabold self-center bg-emerald-500 shadow-[rgba(0,0,0,0.19)_0px_2px_7px_1px] box-border caret-transparent flex justify-center leading-[23px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-center w-full px-2.5 py-5 rounded-[10px] font-montserrat md:text-[19px] md:leading-[23.16px] md:w-[367px]"
                    >
                      Get 50% off today only  ➜
                    </a>
                  </div>
                  <div className="box-border caret-transparent flex justify-around min-h-[auto] min-w-[auto] outline-[3px] w-full my-2.5 p-px md:justify-normal md:my-[22px]">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722821080411_cc_payment_options.webp"
                      alt=""
                      className="text-black box-border caret-transparent justify-start max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[276px] my-px md:w-auto"
                    />
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                  <i className="text-[17px] italic box-border caret-transparent leading-[25.5px] outline-[3px] font-montserrat md:text-base md:leading-6">
                    <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-base md:leading-6">
                      Note:
                    </b>
                    Not Available on Amazon or eBay
                  </i>
                </div>
                <div className="items-end bg-white box-border caret-transparent flex justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full mt-5 pl-[5px] pr-0 pt-px rounded-[10px] md:w-auto md:pt-0 md:px-2.5 md:rounded-[5px]">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mb-px pl-2.5 py-2.5 md:mb-0 md:pt-0 md:pb-[7px]">
                    <div className="text-neutral-900 font-bold box-border caret-transparent leading-[19.2px] outline-[3px] text-left mt-px p-px font-montserrat">
                      Doctor Reviewed
                    </div>
                    <div className="text-neutral-900 text-sm font-medium box-border caret-transparent leading-[21px] outline-[3px] text-left mt-2 mb-2.5 p-px font-montserrat md:text-base md:font-normal md:leading-6">
                      &quot;As a knee pain expert, I only want the very best for
                      my patients - therefore, I trust and recommend
                      RejuvaKnee.&quot;
                    </div>
                    <div className="text-neutral-900 text-sm font-medium box-border caret-transparent leading-[21px] outline-[3px] text-left mt-2 mb-2.5 p-px font-montserrat md:text-base md:font-normal md:leading-6">
                      <i className="text-[13px] italic font-medium box-border caret-transparent leading-[21px] outline-[3px] md:text-base md:font-normal md:leading-6">
                        Dr. James Barkley, Joint Health expert
                      </i>
                    </div>
                  </div>
                  <div className="items-center bg-[url('/lp-images-files-videos-fonts/images/rejuvacare/1722821367050_dr_james_barkley.webp"></div>
                </div>
                <div className="bg-white bg-[url('/lp-images-files-videos-fonts/images/rejuvacare/1713781536653_1713532949416_2_1_.png"></div>
              </div>
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full md:w-[70%]">
                <div className="relative box-border caret-transparent outline-[3px] w-full overflow-hidden before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:text-base before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-6 before:list-outside before:list-disc before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:pt-[100%] before:border-separate before:font-apple_system">
                  <video
                    loop
                    autoPlay
                    playsInline
                    muted
                    src="https://cdn.shopify.com/videos/c/o/v/c986f679e75e415bbf16e2d849473ecb.mp4"
                    poster="/lp-images-files-videos-fonts/images/rejuvacare/1728695465026_avoid_knee_surgery_poster.webp"
                    className="absolute box-border caret-transparent basis-[0%] shrink-0 h-full max-w-full outline-[3px] align-baseline w-full mx-0 px-0 rounded-none left-0 inset-y-0 md:mx-px md:px-px md:rounded-[15px]"
                  ></video>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-white box-border caret-transparent basis-px grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-[15px] pb-2.5 px-2.5 md:pt-[50px]">
              <div className="text-zinc-900 text-[19px] font-bold box-border caret-transparent leading-[24.7px] outline-[3px] p-px font-montserrat">
                As seen on
              </div>
              <div className="box-border caret-transparent h-full outline-[3px] w-full px-px py-0 md:w-[85%] md:px-2.5 md:py-3">
                <img
                  title=""
                  src="/lp-images-files-videos-fonts/images/rejuvacare/1723008768398_trusted_by_brands_mobile.webp"
                  alt=""
                  className="text-black box-border caret-transparent inline h-auto max-w-full outline-[3px] w-full px-px md:h-full md:w-auto md:px-0"
                />
              </div>
            </div>
          </div>
          <div className="relative items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] z-10 md:flex-nowrap">
            <div className="relative bg-sky-100 box-border caret-transparent flex basis-full flex-col grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-[30px] pb-10 px-2.5 md:basis-0 md:px-[100px] md:py-[50px]">
              <div className="text-neutral-900 text-[33px] font-bold box-border caret-transparent leading-[42.9px] min-h-[auto] min-w-[auto] outline-[3px] text-center p-px font-montserrat">
                <div className="text-2xl box-border caret-transparent leading-[30px] outline-[3px] text-center md:text-[33px] md:leading-[42.9px] md:text-left">
                  <span className="text-[23px] box-border caret-transparent leading-[29.9px] outline-[3px] text-center md:text-[33px] md:leading-[42.9px]"></span>
                  <div className="box-border caret-transparent hidden leading-[30px] outline-[3px] text-center p-px md:block md:leading-[42.9px]">
                    Experience Immediate Relief From Stabbing Knee Pain
                    <div className="box-border caret-transparent leading-[30px] outline-[3px] md:leading-[42.9px]">
                      Without Invasive Surgeries or Endless Painkillers
                    </div>
                  </div>
                  <div className="text-2xl box-border caret-transparent block leading-8 outline-[3px] text-center px-2.5 py-px md:text-[33px] md:hidden md:leading-[42.9px] md:px-px">
                    Fast, Effective Relief From Knee Pain, Stiffness, And
                    Swelling That
                    <u className="text-2xl box-border caret-transparent leading-8 outline-[3px] underline md:text-[33px] md:leading-[42.9px]">
                      Lasts All Day
                    </u>
                  </div>
                </div>
              </div>
              <div className="items-stretch box-border caret-transparent flex flex-col justify-around min-h-[auto] min-w-[auto] outline-[3px] w-full mt-[25px] md:flex-row">
                <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-0 md:flex-row md:py-3">
                  <div className="bg-white shadow-[rgba(9,65,60,0.1)_0px_4px_13px_0px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mr-0 px-3.5 py-[18px] rounded-[10px] md:mr-5 md:px-6 md:py-12">
                    <div className="box-border caret-transparent outline-[3px]">
                      <video
                        loop
                        autoPlay
                        playsInline
                        muted
                        src="https://cdn.shopify.com/videos/c/o/v/81e3c0d379624f50a5d7a349f3dd6267.mp4"
                        className="box-border caret-transparent inline h-[130px] max-w-full outline-[3px] align-baseline w-[130px] rounded-[50%]"
                      ></video>
                    </div>
                    <div className="box-border caret-transparent outline-[3px] w-full mt-2">
                      <div className="text-neutral-800 text-[19px] font-bold box-border caret-transparent leading-[26.6px] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-center">
                          <img
                            title=""
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1713536915312_12_1_.png"
                            alt=""
                            className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-5 mr-[5px] mb-[3px]"
                          />
                           Reduces Knee Pain
                        </div>
                      </div>
                    </div>
                    <div className="text-neutral-900 box-border caret-transparent leading-[22.4px] outline-[3px] text-center mt-2 p-px font-montserrat">
                      Provides fast Relief From Knee Pain
                    </div>
                  </div>
                  <div className="bg-white shadow-[rgba(9,65,60,0.1)_0px_4px_13px_0px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mr-0 mt-5 px-3.5 py-[18px] rounded-[10px] md:mr-px md:mt-0 md:px-6 md:py-12">
                    <div className="box-border caret-transparent outline-[3px]">
                      <video
                        loop
                        autoPlay
                        playsInline
                        muted
                        src="https://cdn.shopify.com/videos/c/o/v/3c556bec4f5743f0955e6aca32c9aa3c.mp4"
                        className="box-border caret-transparent inline h-[130px] max-w-full outline-[3px] align-baseline w-[130px] rounded-[50%]"
                      ></video>
                    </div>
                    <div className="box-border caret-transparent outline-[3px] w-full mt-2">
                      <div className="text-neutral-800 text-[19px] font-bold box-border caret-transparent leading-[26.6px] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-center">
                          <img
                            title=""
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1713536915312_12_1_.png"
                            alt=""
                            className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-5 mr-[5px] mb-[3px]"
                          />
                           Boosts Blood Flow
                        </div>
                      </div>
                    </div>
                    <div className="text-neutral-900 box-border caret-transparent leading-[22.4px] outline-[3px] text-center mt-2 p-px font-montserrat">
                      Improves Blood Flow Around Your Knees
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-0 md:flex-row md:py-3">
                  <div className="bg-white shadow-[rgba(9,65,60,0.1)_0px_4px_13px_0px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mr-0 mt-5 px-3.5 py-[18px] rounded-[10px] md:mr-5 md:mt-0 md:px-6 md:py-12">
                    <div className="box-border caret-transparent outline-[3px]">
                      <video
                        loop
                        autoPlay
                        playsInline
                        muted
                        src="https://cdn.shopify.com/videos/c/o/v/d3d483217eb94477922a2d9c28e346a5.mp4"
                        className="box-border caret-transparent inline h-[130px] max-w-full outline-[3px] align-baseline w-[130px] rounded-[50%]"
                      ></video>
                    </div>
                    <div className="box-border caret-transparent outline-[3px] w-full mt-2">
                      <div className="text-neutral-800 text-[19px] font-bold box-border caret-transparent leading-[26.6px] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-center">
                          <img
                            title=""
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1713536915312_12_1_.png"
                            alt=""
                            className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-5 mr-[5px] mb-[3px]"
                          />
                           Proven Technology
                        </div>
                      </div>
                    </div>
                    <div className="text-neutral-900 box-border caret-transparent leading-[22.4px] outline-[3px] text-center mt-2 p-px font-montserrat">
                      Uses Advanced &quot;Triple Method&quot; Technology
                    </div>
                  </div>
                  <div className="bg-white shadow-[rgba(9,65,60,0.1)_0px_4px_13px_0px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mt-5 px-3.5 py-[18px] rounded-[10px] md:mt-0 md:px-6 md:py-12">
                    <div className="box-border caret-transparent outline-[3px]">
                      <video
                        loop
                        autoPlay
                        playsInline
                        muted
                        src="https://cdn.shopify.com/videos/c/o/v/6899745ed0404345968da3d24306a44d.mp4"
                        className="box-border caret-transparent inline h-[130px] max-w-full outline-[3px] align-baseline w-[130px] rounded-[50%]"
                      ></video>
                    </div>
                    <div className="box-border caret-transparent outline-[3px] w-full mt-2">
                      <div className="text-neutral-800 text-[19px] font-bold box-border caret-transparent leading-[26.6px] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-center">
                          <img
                            title=""
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1713536915312_12_1_.png"
                            alt=""
                            className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-5 mr-[5px] mb-[3px]"
                          />
                           Trusted By Over 100,000+ People
                        </div>
                      </div>
                    </div>
                    <div className="text-neutral-900 box-border caret-transparent leading-[22.4px] outline-[3px] text-center mt-2 p-px font-montserrat">
                      <div className="box-border caret-transparent outline-[3px]">
                        Your own in-home Physiotherapist
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-[url('/lp-images-files-videos-fonts/images/rejuvacare/1722936415316_bg_gradient.webp">
              <div className="items-center box-border caret-transparent flex flex-col outline-[3px] w-full p-px md:flex-row-reverse">
                <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full pl-0 py-px md:pl-5">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-left mb-2.5 pr-[30px] font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:min-h-0 md:min-w-0 md:mb-0 md:pr-0">
                    The End To Stiff, Painful, Arthritic Knees And Tendon Issues
                  </div>
                  <div className="relative box-border caret-transparent flex flex-col-reverse min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden mt-2.5 rounded-[15px] md:flex-col md:mt-0 before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:text-base before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-6 before:list-outside before:list-disc before:min-h-[auto] before:min-w-[auto] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:pt-[100%] before:border-separate before:font-apple_system">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/9d9035fc73f84f1e9b88fff4923b298a.mp4"
                      poster="/lp-images-files-videos-fonts/images/rejuvacare/1728697926859_crippling_pain_poster.webp"
                      className="absolute box-border caret-transparent h-auto max-w-full outline-[3px] align-baseline w-full rounded-[15px] left-0 inset-y-0 md:h-full"
                    ></video>
                  </div>
                </div>
                <div className="self-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mr-px mb-[15px] pr-0 md:mr-0 md:mb-0 md:pr-2.5">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      The End To Stiff, Painful, Arthritic Knees And Tendon
                      Issues
                    </div>
                  </div>
                  <div className="text-neutral-900 text-[17px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left mt-2.5 pt-2.5 pb-px px-px font-montserrat md:pb-2.5">
                    <div className="box-border caret-transparent outline-[3px]">
                      If you&#39;re reading this chances are you are suffering
                      from
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        crippling knee pain that won&#39;t subside...
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      The result? Pain dictates your everyday life...
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      Long-term, this can lead to more serious issues like
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        surgery, injections, a loss of independence, and even
                        permanent immobility.
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      Using the &quot;Triple Method&quot; healing approach you
                      can reverse this pain and
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        restore your knee health in just 15 minutes per day
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-white box-border caret-transparent basis-full grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-0 pb-[30px] px-[15px] md:basis-0 md:px-[100px] md:py-5">
              <div className="items-center box-border caret-transparent block flex-col-reverse outline-[3px] w-full mt-px p-px md:flex md:flex-row md:mt-6">
                <div className="box-border caret-transparent min-h-0 min-w-0 outline-[3px] w-full py-px md:min-h-[auto] md:min-w-[auto]">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] outline-[3px] text-left mb-5 font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:mb-0">
                    RejuvaKnee Features 3 Healing Methods To Alleviate Pain In
                    Minutes
                  </div>
                  <div className="relative box-border caret-transparent outline-[3px] w-full overflow-hidden mt-2.5 md:mt-px before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:text-base before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-6 before:list-outside before:list-disc before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:pt-[100%] before:border-separate before:font-apple_system">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/f87276f9c80c47d49b99da496add8a00.mp4"
                      poster="/lp-images-files-videos-fonts/images/rejuvacare/1728700717503_alleviate_pain_in_minutes.webp"
                      className="absolute box-border caret-transparent h-full max-w-full outline-[3px] align-baseline w-full rounded-[15px] left-0 inset-y-0"
                    ></video>
                  </div>
                </div>
                <div className="self-center box-border caret-transparent flex flex-col min-h-0 min-w-0 outline-[3px] w-full ml-px p-0 md:min-h-[auto] md:min-w-[auto] md:ml-0 md:pl-[30px] md:pr-2.5 md:py-3">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      RejuvaKnee Features 3 Healing Methods To Alleviate Pain In
                      Minutes
                    </div>
                  </div>
                  <div className="text-neutral-900 text-[17px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left mt-2.5 px-px py-2.5 font-montserrat">
                    <div className="box-border caret-transparent outline-[3px]">
                      When you put RejuvaKnee’s “Triple Method” Massager on, it
                      uses 3 specific techniques, but
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        most importantly
                      </b>
                      - It uses them at the same time…
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                  </div>
                  <div className="text-neutral-900 text-[17px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left mt-2.5 px-px py-2.5 font-montserrat">
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        The combination of these 3 techniques amplifies their
                        effects
                      </b>
                      boosting collagen production by over 300% and giving your
                      knees the building blocks they need to restore cartilage
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full px-0 py-3 md:px-2.5">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722826373249_num_circle_1.webp"
                        alt=""
                        className="text-black box-border caret-transparent inline max-w-full outline-[3px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        <b className="font-bold box-border caret-transparent outline-[3px]">
                          Soothing Heat :
                        </b>
                        <div className="box-border caret-transparent outline-[3px]"></div>
                        <div className="box-border caret-transparent outline-[3px]">
                          Heat boosts circulation and brings healing nutrients
                          to damaged cartilage. The warmth relaxes the muscles
                          and tendons around the joint, making them more pliable
                          and less prone to pain.
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full px-0 py-3 md:px-2.5">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722826366022_num_circle_2.webp"
                        alt=""
                        className="text-black box-border caret-transparent inline max-w-full outline-[3px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        <b className="font-bold box-border caret-transparent outline-[3px]">
                          Therapeutic Massage :
                        </b>
                        <br className="box-border caret-transparent outline-[3px]" />
                        <div className="box-border caret-transparent outline-[3px]"></div>
                        <div className="box-border caret-transparent outline-[3px]">
                          Micro-vibration nodes mimic the hands of a massage
                          therapist to relax muscles, ease stiffness, and
                          promote joint lubrication. Usually, you would need to
                          go to a clinic to achieve these benefits but with
                          RejuvaKnee you can get them from the comfort of your
                          own home.
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full px-0 py-3 md:px-2.5">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722826593777_num_circle_3.webp"
                        alt=""
                        className="text-black box-border caret-transparent inline max-w-full outline-[3px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        <b className="font-bold box-border caret-transparent outline-[3px]">
                          Dynamic Compression :
                        </b>
                        <br className="box-border caret-transparent outline-[3px]" />
                        <div className="box-border caret-transparent outline-[3px]"></div>
                        <div className="box-border caret-transparent outline-[3px]">
                          Finally, you have dynamic compression, which applies
                          the ideal amount of pressure, relieving stress on the
                          joint and allowing the cartilage to regenerate. This
                          technique acts as the “Amplifier” or the “Glue” that
                          boosts the effects of all three techniques when
                          combined giving you almost complete relief and
                          massively increasing mobility
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-sky-100 box-border caret-transparent basis-full grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-5 pb-10 px-[15px] md:basis-0 md:px-[100px] md:py-[50px]">
              <div className="items-center box-border caret-transparent flex flex-col-reverse outline-[3px] w-full p-px md:[align-items:normal] md:flex-row">
                <div className="self-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mr-px mb-2.5 pr-0 py-0 md:mr-0 md:mb-0 md:pr-2.5 md:py-3">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      Professionally Designed &amp; Recommended
                    </div>
                  </div>
                  <div className="text-[17px] box-border caret-transparent leading-[25.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left mt-5 p-px font-montserrat md:text-lg md:leading-[27px] md:mt-0">
                    Physical therapists aren’t always in arms reach and can be
                    very expensive.
                    <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                      <br className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]" />
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        The RejuvaKnee &quot;Triple Method&quot; Massager has
                        been
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          designed by a leading joint expert as an effective way
                          to get pain relief
                        </b>
                        when you need it most.
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <br className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]" />
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          For less than 1/3 of the cost of one therapist
                          treatment,
                        </b>
                        you can get results that last a lifetime.
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mt-2.5 pl-px py-px md:mt-0 md:pl-5">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-left mb-5 font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:min-h-0 md:min-w-0">
                    Professionally Designed &amp; Recommended
                  </div>
                  <img
                    title=""
                    src="/lp-images-files-videos-fonts/images/rejuvacare/1722828163832_professionally_designed_and_recommended.webp"
                    alt=""
                    className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] rounded-[15px]"
                  />
                </div>
              </div>
              <div className="items-center box-border caret-transparent flex flex-col-reverse outline-[3px] w-full mt-px p-px md:items-start md:flex-row-reverse md:mt-[60px]">
                <div className="self-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full ml-px pl-px pr-0 py-0 md:ml-0 md:pl-[30px] md:pr-2.5 md:py-3">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      The Perfect Therapy Every Time
                    </div>
                  </div>
                  <div className="text-neutral-900 text-[17px] box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left ml-px mt-2.5 px-px py-2.5 font-montserrat md:ml-0">
                    <div className="box-border caret-transparent outline-[3px]">
                      Thousands across the country are posting real results
                      online showing
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        how they avoided surgery after being told it was their
                        only option..
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        <br className="box-border caret-transparent outline-[3px]" />
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        Our next success story could be you!
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      Imagine within a few short weeks of receiving your
                      RejuvaKnee,
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        you could amaze all your friends and family with how
                        your life has changed - what would that look like for
                        you?
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      Imagine all the things you could do that were previously
                      “off limits” due to knee pain… 
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        Imagine:
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <b className="font-bold box-border caret-transparent outline-[3px]"></b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]"></div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full ml-px pl-px md:ml-0 md:pl-0">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722828438493_icon_ex_circle.svg"
                        alt=""
                        className="text-black box-border caret-transparent inline h-full max-w-full outline-[3px] w-[35px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] text-left p-px font-montserrat md:text-base md:leading-6">
                        No more struggling to walk or climb up and down stairs
                      </div>
                    </div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full ml-px mt-2.5 pl-px md:ml-0 md:pl-0">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722828438493_icon_ex_circle.svg"
                        alt=""
                        className="text-black box-border caret-transparent inline h-full max-w-full outline-[3px] w-[35px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] text-left p-px font-montserrat md:text-base md:leading-6">
                        No more feeling like a burden or feeling like you are
                        losing your independence 
                      </div>
                    </div>
                  </div>
                  <div className="items-start box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full ml-px mt-2.5 pl-px md:ml-0 md:pl-0">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px]">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722828438493_icon_ex_circle.svg"
                        alt=""
                        className="text-black box-border caret-transparent inline h-full max-w-full outline-[3px] w-[35px] pr-2"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full pl-[5px]">
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] text-left p-px font-montserrat md:text-base md:leading-6">
                        No more sleepless nights trying to find a comfortable
                        position where your knees aren&#39;t in agony
                      </div>
                    </div>
                  </div>
                  <div className="text-[17px] box-border caret-transparent leading-[25.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left mt-5 p-px font-montserrat md:text-base md:leading-6">
                    With just 15 minutes per day, RejuvaKnee can make this your
                    reality and give you another shot at a normal life
                  </div>
                  <div className="box-border caret-transparent block min-h-[auto] min-w-[auto] outline-[3px] w-full mt-4 p-px md:flex">
                    <a
                      title="Get up to 50% off today only  "
                      className="text-white text-xl font-extrabold self-center bg-emerald-500 shadow-[rgba(0,0,0,0.19)_0px_2px_7px_1px] box-border caret-transparent flex justify-center leading-[23px] max-w-full min-h-0 min-w-0 outline-[3px] text-center w-full px-2.5 py-5 rounded-[10px] font-montserrat md:text-[19px] md:leading-[23.16px] md:min-h-[auto] md:min-w-[auto]"
                    >
                      Get 50% off today only  ➜
                    </a>
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mt-5 py-px md:mt-0">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] outline-[3px] text-left mb-5 pr-[50px] font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:mb-0 md:pr-0">
                    The Perfect Therapy Every Time
                  </div>
                  <div className="relative box-border caret-transparent outline-[3px] w-full overflow-hidden rounded-[15px] before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:text-base before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-6 before:list-outside before:list-disc before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:pt-[100%] before:border-separate before:font-apple_system">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/bf8b375446284d91a5ac1600b3d0f9f5.mp4"
                      poster="/lp-images-files-videos-fonts/images/rejuvacare/1728702670063_the_perfect_therapy_poster.webp"
                      className="absolute box-border caret-transparent h-auto max-w-full outline-[3px] align-baseline w-full rounded-[15px] left-0 inset-y-0 md:h-full"
                    ></video>
                    <div className="box-border caret-transparent hidden outline-[3px] w-full mt-4 p-px">
                      <a
                        title="Get up to 50% off today only  "
                        className="text-white text-xl font-extrabold self-center bg-emerald-500 shadow-[rgba(0,0,0,0.19)_0px_2px_7px_1px] box-border caret-transparent flex justify-center leading-[23px] max-w-full outline-[3px] text-center w-full px-2.5 py-5 rounded-[10px] font-montserrat md:text-[19px] md:leading-[23.16px]"
                      >
                        Get 50% off today only  ➜
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-white box-border caret-transparent basis-full flex-col grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-5 pb-[25px] px-[15px] md:basis-0 md:flex-row md:px-[100px] md:py-[50px]">
              <div className="items-center box-border caret-transparent flex flex-col-reverse justify-start outline-[3px] w-full mt-px p-px md:[align-items:normal] md:flex-row md:justify-normal md:mt-0">
                <div className="self-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mr-px pr-0 py-0 md:mr-0 md:pr-2.5 md:py-3">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      How RejuvaKnee &quot;Triple Method&quot; Massager Works
                    </div>
                  </div>
                  <div className="text-[17px] box-border caret-transparent leading-[25.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left mt-5 mb-2.5 p-px font-montserrat md:text-lg md:leading-[27px] md:my-0">
                    The best part about the RejuvaKnee Massager is that it’s
                    <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                      so simple to use.
                    </b>
                    <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                      <br className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]" />
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                        <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                        <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        First, there are
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          no annoying cords
                        </b>
                        - You just charge it with the electricity plug and cable
                        that arrives together with your order.
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <br className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]" />
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        The second step is to
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          slip RejuvaKnee on your knee
                        </b>
                        and then use the touch screen to adjust the intensity of
                        heating and massage.
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          Sit back and relax while RejuvaKnee does its magic.
                        </b>
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <br className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]" />
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                        <b className="text-[17px] font-bold box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]">
                          With consistent use, the damage to your knee cartilage
                          can be reversed,
                        </b>
                        providing long-lasting relief and allowing you to get
                        back to living your life to the fullest without being
                        held back by crippling knee pain.
                      </div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                      <div className="text-[17px] box-border caret-transparent leading-[25.5px] outline-[3px] md:text-lg md:leading-[27px]"></div>
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mt-2.5 pl-px py-px md:mt-0 md:pl-5">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-left mt-px mb-5 font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:min-h-0 md:min-w-0 md:mt-0">
                    How RejuvaKnee &quot;Triple Method&quot; Massager Works
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-px pt-px md:mt-0 md:pt-0">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1728704835236_how_rejuvaknee_massager_works.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-full rounded-[15px]"
                    />
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent flex flex-col-reverse justify-center outline-[3px] w-full mt-5 p-px md:[align-items:normal] md:flex-row-reverse md:justify-normal md:mt-[60px]">
                <div className="self-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mx-px p-0 md:mx-0 md:pl-[30px] md:pr-2.5 md:py-3">
                  <div className="text-stone-700 text-[32px] font-extrabold box-border caret-transparent leading-[38.4px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-open_sans md:text-[40px] md:leading-[48px]">
                    <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent hidden leading-[33px] outline-[3px] font-montserrat md:text-[33px] md:block md:leading-[42px]">
                      Try It for 90 Days, 100% Risk-Free
                    </div>
                  </div>
                  <div className="text-neutral-900 text-[17px] box-border caret-transparent mt-[-7px] min-h-[auto] min-w-[auto] outline-[3px] text-left pt-px pb-2.5 px-px font-montserrat md:mt-2.5 md:pt-2.5">
                    <div className="box-border caret-transparent outline-[3px]">
                      Here’s the deal: We believe in this product so much that
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        we don’t want you to spend a dime until you are 100%
                        certain that it works for you.
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      That’s why we are offering a
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        90-day “Results or Refund” guarantee.
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      Simply, get yourself a RejuvaKnee &quot;Triple
                      Method&quot; Massager today, use it for 90 days, and then
                      make your decision based on the results.
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        <u className="box-border caret-transparent outline-[3px] underline">
                          If you are not absolutely thrilled with the results,
                          we will issue you an immediate refund.
                        </u>
                      </b>
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      <br className="box-border caret-transparent outline-[3px]" />
                    </div>
                    <div className="box-border caret-transparent outline-[3px]">
                      In other words, you are only paying if it turns out to be
                      a complete lifesaver.
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mt-px pr-px py-px md:mt-0 md:pr-0">
                  <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent block leading-[33px] outline-[3px] text-left mb-5 pr-[50px] font-montserrat md:text-[33px] md:hidden md:leading-[42px] md:mb-0 md:pr-0">
                    Try It for 90 Days, 100% Risk-Free
                  </div>
                  <div className="box-border caret-transparent outline-[3px]">
                    <video
                      src="https://cdn.shopify.com/videos/c/o/v/11af497fc07f477ab54624aa05401062.mp4"
                      loop
                      autoPlay
                      playsInline
                      muted
                      className="box-border caret-transparent inline max-w-full outline-[3px] align-baseline rounded-t-[15px]"
                    ></video>
                    <div className="box-border caret-transparent h-[230px] mt-[-7px] outline-[3px] overflow-hidden rounded-b-[15px]">
                      <video
                        loop
                        autoPlay
                        playsInline
                        muted
                        src="https://cdn.shopify.com/videos/c/o/v/ecf002cebaa144d1b860ed657ad3697c.mp4"
                        className="box-border caret-transparent inline mt-[-25px] max-w-full outline-[3px] align-baseline w-full rounded-b-[15px]"
                      ></video>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-center bg-sky-100 box-border caret-transparent flex basis-full flex-col grow justify-center max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto px-2.5 py-5 md:basis-0 md:justify-around md:px-[100px] md:py-[50px]">
              <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-center font-montserrat md:text-[33px] md:leading-[42px]">
                Hear It From Our Customers
              </div>
              <img
                title=""
                src="/lp-images-files-videos-fonts/images/rejuvacare/1722832035148_5_stars_yellow.png"
                alt=""
                className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[110px] my-2.5 md:w-[150px] md:mb-[30px]"
              />
              <div className="items-start box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full md:flex-row">
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-1.5 py-3 md:px-2.5">
                  <div className="items-start bg-white shadow-[rgba(0,0,0,0.12)_2px_2px_8px_2px] box-border caret-transparent flex flex-col justify-center outline-[3px] w-full rounded-[15px]">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722832434914_silvia_m.webp"
                      alt=""
                      className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-t-[15px]"
                    />
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-5 md:px-[25px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        &quot;I bought 2 of these online while scrolling social
                        media. It was an impulse buy. I admit it. I didn’t
                        believe in it much at first. Once they arrived after
                        slightly delayed shipping I tried it for a week and felt
                        just a bit of relief. I decided to give it another week,
                        just because of all the the positive reviews. And that’s
                        when I started to notice a real difference. Another 2
                        weeks passed by and my knee pain is completely gone!
                        Like it was never there.&quot;
                        <div className="box-border caret-transparent outline-[3px]">
                          <br className="box-border caret-transparent outline-[3px]" />
                          <div className="box-border caret-transparent outline-[3px]"></div>
                          <div className="box-border caret-transparent outline-[3px]">
                            <b className="font-bold box-border caret-transparent outline-[3px]">
                              Silvia M.
                            </b>
                          </div>
                        </div>
                      </div>
                      <div className="box-border caret-transparent flex outline-[3px]">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722832035148_5_stars_yellow.png"
                          alt=""
                          className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px]"
                        />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-1.5 py-3 md:px-2.5">
                  <div className="items-start bg-white shadow-[rgba(0,0,0,0.1)_2px_2px_8px_2px] box-border caret-transparent flex flex-col justify-center outline-[3px] w-full rounded-[15px]">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1723112828479_andrew_p_middle_card_review.png"
                      alt=""
                      className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-t-[15px]"
                    />
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-5 md:px-[25px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px]"></div>
                        <div className="box-border caret-transparent outline-[3px]">
                          &quot;I was not sure about the long lasting effects
                          but decided to try it because I have suffered from
                          knee pain for a while now. I take OTC med for the pain
                          but if you forget to take one dose the pain will
                          remind you it is still there. The instructions were
                          easy to follow and RejuvaKnee even has a video that
                          shows how to use it and how to place it onto your
                          knee. The first day I tried it I let it go through its
                          cycle and, honestly, it felt very good. After it
                          finished the pain was gone. After a while you forget
                          you had any pain. I would have taken about four pills
                          in the same time period without my new knee massager!
                          I would recommend this massager to anyone with chronic
                          knee pain. This thing works!&quot;
                        </div>
                        <div className="box-border caret-transparent outline-[3px]">
                          <br className="box-border caret-transparent outline-[3px]" />
                        </div>
                        <div className="box-border caret-transparent outline-[3px]">
                          <b className="font-bold box-border caret-transparent outline-[3px]">
                            Andrew P.
                          </b>
                        </div>
                      </div>
                      <div className="box-border caret-transparent flex outline-[3px]">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722832035148_5_stars_yellow.png"
                          alt=""
                          className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px]"
                        />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-1.5 py-3 md:px-2.5">
                  <div className="items-start bg-white shadow-[rgba(0,0,0,0.1)_2px_2px_8px_2px] box-border caret-transparent flex flex-col justify-center outline-[3px] w-full rounded-[15px]">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722832418378_bridget_f.webp"
                      alt=""
                      className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-t-[15px]"
                    />
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-5 md:px-[25px]">
                      <div className="box-border caret-transparent outline-[3px] text-left p-px font-montserrat">
                        &quot;I have endured knee pain for at least 20 years. A
                        physical therapist has been my only relief and that of
                        course is short-lived. I must say, I was skeptical when
                        I ordered RejuvaKnee. I can now say, no doubt about it,
                        this thing works! Remains to be seen what the long-term
                        effects will be, however, I am optimistic and looking
                        forward to being able to walk through the day without
                        pain pills. So far the only negative is that my husband
                        keeps stealing it from me.&quot;
                        <div className="box-border caret-transparent outline-[3px]"></div>
                        <div className="box-border caret-transparent outline-[3px]">
                          <br className="box-border caret-transparent outline-[3px]" />
                        </div>
                        <div className="box-border caret-transparent outline-[3px]">
                          <b className="font-bold box-border caret-transparent outline-[3px]">
                            Bridget F.
                          </b>
                        </div>
                      </div>
                      <div className="box-border caret-transparent flex outline-[3px]">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722832035148_5_stars_yellow.png"
                          alt=""
                          className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px]"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full"></div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-center bg-white box-border caret-transparent block basis-px grow justify-center max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto px-0 py-5 md:flex md:px-[100px] md:py-[50px]">
              <div className="box-border caret-transparent min-h-0 min-w-0 outline-[3px] w-full p-0 md:min-h-[auto] md:min-w-[auto] md:px-2.5 md:py-3">
                <img
                  title=""
                  src="/lp-images-files-videos-fonts/images/rejuvacare/1723110728696_rejuvaknee_kit_desktop2.png"
                  alt=""
                  className="text-black box-border caret-transparent inline h-auto max-w-full object-cover outline-[3px] w-full md:h-full md:object-fill"
                />
              </div>
              <div className="box-border caret-transparent min-h-0 min-w-0 outline-[3px] w-full px-[15px] py-3 md:min-h-[auto] md:min-w-[auto] md:px-2.5">
                <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent leading-[33px] outline-[3px] text-left font-montserrat md:text-[33px] md:leading-[42px]">
                  Inside The Treatment Kit
                </div>
                <div className="items-start box-border caret-transparent flex outline-[3px] w-full mt-5 px-2.5">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px] pr-2">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722826373249_num_circle_1.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[35px]"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="text-lg box-border caret-transparent leading-[27px] outline-[3px] text-left pl-[5px] pr-px py-px font-montserrat">
                      <div className="text-base box-border caret-transparent leading-6 outline-[3px] md:text-lg md:leading-[27px]">
                        High Quality Gift Box
                      </div>
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex outline-[3px] w-full mt-2.5 px-2.5">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px] pr-2">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722826366022_num_circle_2.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[35px]"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="text-lg box-border caret-transparent leading-[27px] outline-[3px] text-left pl-[5px] pr-px py-px font-montserrat">
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="text-base box-border caret-transparent leading-[22px] outline-[3px] md:text-lg md:leading-[27px]">
                        The RejuvaKnee &quot;Triple Method&quot; Massager
                      </div>
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex outline-[3px] w-full mt-2.5 px-2.5">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px] pr-2">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722826593777_num_circle_3.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[35px]"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="text-lg box-border caret-transparent leading-[27px] outline-[3px] text-left pl-[5px] pr-px py-px font-montserrat">
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="text-base box-border caret-transparent leading-[22px] outline-[3px] md:text-lg md:leading-[27px]">
                        User Guide (set up within 30 seconds)
                      </div>
                    </div>
                  </div>
                </div>
                <div className="items-start box-border caret-transparent flex outline-[3px] w-full mt-2.5 px-2.5">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[35px] pr-2">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722833788239_num_circle_4.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[35px]"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="text-lg box-border caret-transparent leading-[27px] outline-[3px] text-left pl-[5px] pr-px py-px font-montserrat">
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="text-base box-border caret-transparent leading-6 outline-[3px] md:text-lg md:leading-[27px]">
                        Your RejuvaKnee USB C Charger
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-center bg-emerald-500 box-border caret-transparent block basis-px flex-col grow justify-center max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-0 pb-5 px-2.5 md:flex md:flex-row md:pl-[30px] md:pr-[100px] md:py-[50px]">
              <div className="static bg-white box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] w-2/5 z-auto ml-px px-[25px] py-5 rounded-[15px] right-auto md:relative md:block md:min-h-[auto] md:min-w-[auto] md:right-[-100px] md:z-[1]">
                <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent leading-[33px] outline-[3px] text-left font-montserrat md:text-[33px] md:leading-[42px]">
                  Finish setting up in 30 seconds
                </div>
                <div className="box-border caret-transparent outline-[3px] text-left mt-5 p-px font-montserrat">
                  The best part about RejuvaKnee Massager is that it’s so simple
                  to use. There are no annoying cords - simply charge your
                  device using the plug and cable that come in your kit.
                </div>
              </div>
              <div className="relative bg-white box-border caret-transparent block outline-[3px] w-full z-0 ml-px mt-px pt-5 pb-8 px-[25px] rounded-[15px] top-[30px] md:static md:hidden md:w-2/5 md:z-auto md:mt-0 md:pb-5 md:top-auto">
                <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent leading-[33px] outline-[3px] text-left pr-5 font-montserrat md:text-[33px] md:leading-[42px] md:pr-0">
                  Finish setting up within 30 seconds
                </div>
                <div className="box-border caret-transparent outline-[3px] text-left mt-5 p-px font-montserrat">
                  The best part about RejuvaKnee Massager is that it’s so simple
                  to use. There are no annoying cords - simply charge your
                  device using the plug and cable that come in your kit.
                </div>
              </div>
              <div className="box-border caret-transparent min-h-0 min-w-0 outline-[3px] w-full px-2.5 py-3 md:min-h-[auto] md:min-w-[auto]">
                <div className="box-border caret-transparent outline-[3px] rounded-[15px]">
                  <video
                    src="https://cdn.shopify.com/videos/c/o/v/f3561b42fa794e4abd7e94375cf2f506.mp4"
                    controls
                    poster="/lp-images-files-videos-fonts/images/rejuvacare/43.jpg"
                    className="relative box-border caret-transparent inline max-w-full outline-[3px] align-baseline w-full rounded-[10px] md:rounded-[15px]"
                  ></video>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-center bg-white box-border caret-transparent block basis-px flex-col grow justify-center max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-[30px] pb-2.5 px-[15px] md:flex md:pt-[50px] md:px-[100px]">
              <div className="items-center box-border caret-transparent flex flex-col justify-center min-h-0 min-w-0 outline-[3px] w-auto pt-px px-0 md:min-h-[auto] md:min-w-[auto] md:w-full md:px-2.5">
                <div className="text-neutral-500 text-[19px] font-bold box-border caret-transparent leading-[23.16px] min-h-[auto] min-w-[auto] outline-[3px] text-center font-montserrat md:text-left"></div>
                <div className="text-neutral-500 text-[19px] font-bold box-border caret-transparent leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-center font-montserrat md:text-xl md:leading-[30px]">
                  What makes the
                  <span className="text-emerald-500 text-[19px] box-border caret-transparent leading-[28.5px] outline-[3px] text-center md:text-xl md:leading-[30px]">
                    RejuvaKnee
                  </span>
                  massager special?
                </div>
                <div className="text-[23px] font-bold box-border caret-transparent leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-5 font-montserrat md:text-[33px] md:leading-[42px]">
                  &quot;The RejuvaKnee &#39;Triple Method&#39; Massager Just
                  Works&quot;
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[325px] outline-[3px] w-full mt-[30px] p-0 md:w-4/5 md:px-2.5 md:py-3">
                  <div className="items-center box-border caret-transparent justify-center outline-[3px] w-full md:[align-items:normal] md:justify-normal"></div>
                  <div className="items-center box-border caret-transparent flex outline-[3px] border-stone-300 border-b border-solid">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] p-2.5 md:w-full">
                      <div className="text-white box-border caret-transparent outline-[3px] text-right px-2.5 py-px font-montserrat">
                        spacer
                      </div>
                    </div>
                    <div className="items-stretch box-border caret-transparent flex justify-end min-h-[auto] min-w-[auto] outline-[3px] w-full md:items-center md:justify-normal">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-0 py-[13px] rounded-t-[15px] md:px-2.5">
                        <div className="text-[10px] font-bold box-border caret-transparent leading-[15px] outline-[3px] text-center p-px font-montserrat md:text-base md:leading-6">
                          <span className="text-emerald-500 text-[10px] box-border caret-transparent leading-[15px] outline-[3px] md:text-base md:leading-6">
                            REJUVA
                          </span>
                          KNEE
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full px-0 py-2.5 md:[align-items:normal] md:block md:px-2.5">
                        <div className="box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] mr-px p-px font-montserrat md:leading-[22px] md:min-h-0 md:min-w-0 md:mr-0">
                          <b className="text-[10px] font-bold box-border caret-transparent leading-3 outline-[3px] ml-2.5 md:text-base md:leading-[22px] md:ml-0">
                            Other Brands
                          </b>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex justify-between min-h-[auto] min-w-[auto] outline-[3px] w-[92px] ml-px px-[18px] py-2.5 md:[align-items:normal] md:block md:justify-normal md:w-full md:ml-0 md:px-2.5">
                        <div className="box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat md:leading-[22px] md:min-h-0 md:min-w-0">
                          <b className="text-[10px] font-bold box-border caret-transparent leading-[15px] outline-[3px] md:text-base md:leading-[22px]">
                            Medications
                          </b>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="items-center box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-2.5 pr-0 py-0 md:w-full md:pr-2.5 md:py-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left px-2.5 py-px font-montserrat md:text-base md:leading-6 md:text-right">
                        Lasting pain relief
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center bg-black/0 box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-2.5 pr-0 py-0 md:w-full md:pr-2.5 md:py-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left px-2.5 py-px font-montserrat md:text-base md:leading-6 md:text-right">
                        Drug Free
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-[7px] pr-0 py-1.5 md:w-full md:p-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left px-2.5 py-0 font-montserrat md:text-base md:leading-6 md:text-right md:py-px">
                        Easy to use &amp; carry, wireless, and fully rechargable
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center bg-black/0 box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-[7px] pr-0 py-1.5 md:w-full md:p-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left px-2.5 py-1.5 font-montserrat md:text-base md:leading-6 md:text-right md:py-px">
                        3 in 1 Technology: Compression, heat, &amp; massage
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-2.5 pr-0 py-1.5 md:w-full md:pr-2.5 md:py-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left pl-1.5 pr-2.5 pt-px pb-1.5 font-montserrat md:text-base md:leading-6 md:text-right md:pl-2.5 md:pb-px">
                        Activates body&#39;s self-healing mechanism
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center bg-black/0 box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-2.5 pr-0 py-1.5 md:w-full md:pr-2.5 md:py-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left pl-1.5 pr-2.5 py-px font-montserrat md:text-base md:leading-6 md:text-right md:pl-2.5">
                        Reverses damage in the knee joint 
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] pl-[5px] pr-0 py-1.5 md:w-full md:p-2.5">
                      <div className="text-sm box-border caret-transparent leading-[18px] outline-[3px] text-left px-2.5 py-0 font-montserrat md:text-base md:leading-6 md:text-right md:py-px">
                        Addresses &quot;Join Blockage&quot; The Root Cause
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="items-center bg-black/0 box-border caret-transparent flex outline-[3px] w-full">
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[200px] p-0 md:w-full md:p-2.5">
                      <div className="text-sm box-border caret-transparent leading-[22px] outline-[3px] text-left pl-4 pr-2.5 py-px font-montserrat md:text-base md:leading-6 md:text-right md:pl-2.5">
                        Affordability
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5 rounded-b-[15px]">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                      <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722907474978_icon_ex_circle_gray.webp"
                          alt=""
                          className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-[25px] mx-px"
                        />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="text-[23px] font-bold box-border caret-transparent leading-[32.2px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-6 font-montserrat md:text-[33px] md:leading-[46.2px] md:text-left md:mt-8"></div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-start bg-sky-100 box-border caret-transparent block basis-px grow justify-start max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto px-0 py-5 md:items-center md:flex md:justify-center md:px-[100px] md:py-[50px]">
              <div className="items-center box-border caret-transparent flex flex-col min-h-0 min-w-0 outline-[3px] w-[90%] mx-px px-0 py-3 md:min-h-[auto] md:min-w-[auto] md:w-full md:mx-0 md:px-10">
                <img
                  title=""
                  src="/lp-images-files-videos-fonts/images/rejuvacare/1722844015319_90_day_guarantee.webp"
                  alt=""
                  className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[110px]"
                />
                <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                  90-Day Money Back Guarantee
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat md:mt-0">
                  If you’re not absolutely thrilled with the results, we don’t
                  want your money. No hassles, no questions asked.
                </div>
              </div>
              <div className="items-center box-border caret-transparent flex flex-col min-h-0 min-w-0 outline-[3px] w-[90%] mt-2.5 px-0 py-3 md:min-h-[auto] md:min-w-[auto] md:w-full md:mt-0 md:px-10">
                <img
                  title=""
                  src="/lp-images-files-videos-fonts/images/rejuvacare/1722844006854_free_shipping.webp"
                  alt=""
                  className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[110px]"
                />
                <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                  Free, Tracked Shipping
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat md:mt-0">
                  We provide fast free shipping and handling from US Warehouse
                  on all orders.
                </div>
              </div>
              <div className="items-center box-border caret-transparent flex flex-col min-h-0 min-w-0 outline-[3px] w-[90%] mt-2.5 px-0 py-3 md:min-h-[auto] md:min-w-[auto] md:w-full md:mt-0 md:px-7">
                <img
                  title=""
                  src="/lp-images-files-videos-fonts/images/rejuvacare/1722843985659_guaranteed_high_quality.webp"
                  alt=""
                  className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[110px]"
                />
                <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                  Guaranteed High Quality
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat md:mt-0">
                  RejuvaKnee Massagers are made of high quality materials. Enjoy
                  consistent, trouble-free performance for a long time.
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-start bg-white box-border caret-transparent block basis-px flex-col grow justify-center max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-0 pb-2.5 px-0 md:flex md:flex-row md:px-[100px] md:py-10">
              <div className="items-center box-border caret-transparent flex-col min-h-0 min-w-0 outline-[3px] w-full md:min-h-[auto] md:min-w-[auto] md:w-6/12">
                <div className="items-stretch box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
                  <div className="relative box-border caret-transparent basis-full grow max-w-full min-h-[25px] outline-[3px] w-min md:basis-0">
                    <div className="box-border caret-transparent hidden outline-[3px]">
                      <div
                        className="box-border caret-transparent max-w-[375px] outline-[3px] w-full md:max-w-screen-xl"
                      >
                        <div className="box-border caret-transparent outline-[3px] m-0 md:m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930434548_1.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930428614_2.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930421180_3.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930412931_4.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                      </div>
                      <div
                        className="box-border caret-transparent max-w-[375px] outline-[3px] w-full mx-0 px-0 md:max-w-screen-xl md:mx-auto md:px-[5px]"
                      >
                        <div className="box-border caret-transparent outline-[3px] w-full mx-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930434548_1.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full mx-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930428614_2.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full mx-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930421180_3.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full mx-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930412931_4.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="box-border caret-transparent hidden outline-[3px] m-[5px]"></div>
                    <div className="box-border caret-transparent hidden outline-[3px] m-[5px] pb-[5px] px-[5px]">
                      <div
                        className="box-border caret-transparent max-w-[375px] outline-[3px] w-full mt-[5px] mx-auto pt-[7px] px-[5px] md:max-w-screen-xl md:pt-0"
                      >
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930434548_1.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930428614_2.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-auto md:w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930421180_3.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930412931_4.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                      </div>
                      <div
                        className="box-border caret-transparent max-w-[375px] outline-[3px] w-full mb-[5px] mx-auto px-[5px] md:max-w-screen-xl"
                      >
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930434548_1.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930428614_2.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930421180_3.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                        <div className="box-border caret-transparent outline-[3px] w-full m-[5px] p-[5px]">
                          <img
                            src="/lp-images-files-videos-fonts/images/rejuvacare/1722930412931_4.webp"
                            title=""
                            alt=""
                            className="box-border caret-transparent inline max-w-full outline-[3px] w-full"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="box-border caret-transparent outline-[3px] m-[5px] p-[5px]">
                      <div
                        className="relative box-border caret-transparent max-w-[375px] outline-[3px] w-full mt-[5px] mx-auto px-[5px] md:max-w-screen-xl"
                      >
                        <button
                          type="button"
                          className="static text-black text-[13.3333px] font-normal [align-items:normal] bg-zinc-100 caret-black inline-block h-auto justify-normal leading-[normal] max-w-none outline-0 text-center transform-none w-auto z-auto rounded-none left-auto top-auto md:absolute md:text-[25px] md:font-bold md:items-center md:aspect-auto md:bg-sky-100 md:caret-transparent md:flex md:h-[35px] md:justify-center md:leading-[37.5px] md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:translate-y-[-50.0%] md:w-[35px] md:z-[1] md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto] md:rounded-[50%] md:left-5 md:top-2/4"
                        >
                          <i className="md:font-black md:aspect-auto md:box-border md:caret-transparent md:block md:leading-[25px] md:min-h-[auto] md:min-w-[auto] md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto] md:font-font_awesome_5_free before:md:accent-auto before:md:aspect-auto before:md:box-border before:md:caret-transparent before:md:text-black before:md:text-[25px] before:md:not-italic before:md:normal-nums before:md:font-black before:md:tracking-[normal] before:md:leading-[25px] before:md:list-outside before:md:list-disc before:md:outline-[3px] before:md:overscroll-x-auto before:md:overscroll-y-auto before:md:pointer-events-auto before:md:snap-align-none before:md:snap-normal before:md:snap-none before:md:text-center before:md:no-underline before:md:decoration-auto before:md:indent-[0px] before:md:normal-case before:md:underline-offset-auto before:md:visible before:md:scroll-m-0 before:md:scroll-p-[auto] before:md:border-separate before:md:font-font_awesome_5_free"></i>
                        </button>
                        <div className="static box-content caret-black outline-0 md:relative md:aspect-auto md:box-border md:caret-transparent md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:overflow-hidden md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]">
                          <div className="static box-content caret-black outline-0 transform-none w-auto mx-0 left-auto top-auto md:relative md:aspect-auto md:box-border md:caret-transparent md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:translate-x-[-510px] md:w-[5610px] md:[mask-position:0%] md:bg-left-top md:mx-auto md:scroll-m-0 md:scroll-p-[auto] md:left-0 md:top-0 before:md:accent-auto before:md:aspect-auto before:md:box-border before:md:caret-transparent before:md:text-neutral-800 before:md:table before:md:text-base before:md:not-italic before:md:normal-nums before:md:font-normal before:md:tracking-[normal] before:md:leading-6 before:md:list-outside before:md:list-disc before:md:outline-[3px] before:md:overscroll-x-auto before:md:overscroll-y-auto before:md:pointer-events-auto before:md:snap-align-none before:md:snap-normal before:md:snap-none before:md:no-underline before:md:decoration-auto before:md:indent-[0px] before:md:normal-case before:md:underline-offset-auto before:md:visible before:md:scroll-m-0 before:md:scroll-p-[auto] before:md:border-separate before:md:font-apple_system after:md:accent-auto after:md:aspect-auto after:md:box-border after:md:caret-transparent after:md:clear-both after:md:text-neutral-800 after:md:table after:md:text-base after:md:not-italic after:md:normal-nums after:md:font-normal after:md:tracking-[normal] after:md:leading-6 after:md:list-outside after:md:list-disc after:md:outline-[3px] after:md:overscroll-x-auto after:md:overscroll-y-auto after:md:pointer-events-auto after:md:snap-align-none after:md:snap-normal after:md:snap-none after:md:no-underline after:md:decoration-auto after:md:indent-[0px] after:md:normal-case after:md:underline-offset-auto after:md:visible after:md:scroll-m-0 after:md:scroll-p-[auto] after:md:border-separate after:md:font-apple_system">
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]"></div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[335px] m-[5px] md:w-[500px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974932872_1.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[335px] m-[5px] md:w-[500px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974926450_2.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[335px] m-[5px] md:w-[500px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974920398_3.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[335px] m-[5px] md:w-[500px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974912954_4.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[335px] m-[5px] md:w-[500px]"></div>
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974932872_1.webp"
                                title=""
                                alt=""
                                className="box-content caret-black max-w-none outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-full md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]"
                              />
                            </div>
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974926450_2.webp"
                                title=""
                                alt=""
                                className="box-content caret-black max-w-none outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-full md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]"
                              />
                            </div>
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974920398_3.webp"
                                title=""
                                alt=""
                                className="box-content caret-black max-w-none outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-full md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]"
                              />
                            </div>
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974912954_4.webp"
                                title=""
                                alt=""
                                className="box-content caret-black max-w-none outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-full md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]"
                              />
                            </div>
                            <div className="box-content caret-black float-none min-h-0 outline-0 w-auto md:aspect-auto md:box-border md:caret-transparent md:float-left md:min-h-px md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[500px] md:[mask-position:0%] md:bg-left-top md:m-[5px] md:scroll-m-0 md:scroll-p-[auto]"></div>
                          </div>
                        </div>
                        <button
                          type="button"
                          className="static text-black text-[13.3333px] font-normal [align-items:normal] bg-zinc-100 caret-black inline-block h-auto justify-normal leading-[normal] max-w-none outline-0 text-center transform-none w-auto z-auto rounded-none right-auto top-auto md:absolute md:text-[25px] md:font-bold md:items-center md:aspect-auto md:bg-sky-100 md:caret-transparent md:flex md:h-[35px] md:justify-center md:leading-[37.5px] md:max-w-full md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:translate-y-[-50.0%] md:w-[35px] md:z-[1] md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto] md:rounded-[50%] md:right-5 md:top-2/4"
                        >
                          <i className="md:font-black md:aspect-auto md:box-border md:caret-transparent md:block md:leading-[25px] md:min-h-[auto] md:min-w-[auto] md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto] md:font-font_awesome_5_free before:md:accent-auto before:md:aspect-auto before:md:box-border before:md:caret-transparent before:md:text-black before:md:text-[25px] before:md:not-italic before:md:normal-nums before:md:font-black before:md:tracking-[normal] before:md:leading-[25px] before:md:list-outside before:md:list-disc before:md:outline-[3px] before:md:overscroll-x-auto before:md:overscroll-y-auto before:md:pointer-events-auto before:md:snap-align-none before:md:snap-normal before:md:snap-none before:md:text-center before:md:no-underline before:md:decoration-auto before:md:indent-[0px] before:md:normal-case before:md:underline-offset-auto before:md:visible before:md:scroll-m-0 before:md:scroll-p-[auto] before:md:border-separate before:md:font-font_awesome_5_free"></i>
                        </button>
                      </div>
                      <div
                        className="relative box-border caret-transparent max-w-[375px] outline-[3px] w-full mb-px mx-auto pl-[5px] pr-2.5 md:max-w-screen-xl"
                      >
                        <div className="static box-content caret-black outline-0 md:relative md:aspect-auto md:box-border md:caret-transparent md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:overflow-hidden md:[mask-position:0%] md:bg-left-top md:scroll-m-0 md:scroll-p-[auto]">
                          <div className="static box-content caret-black outline-0 w-auto mx-0 left-auto top-auto md:relative md:aspect-auto md:box-border md:caret-transparent md:outline-[3px] md:overscroll-x-auto md:overscroll-y-auto md:snap-align-none md:snap-normal md:snap-none md:decoration-auto md:underline-offset-auto md:w-[508px] md:[mask-position:0%] md:bg-left-top md:mx-auto md:scroll-m-0 md:scroll-p-[auto] md:left-0 md:top-0 before:md:accent-auto before:md:aspect-auto before:md:box-border before:md:caret-transparent before:md:text-neutral-800 before:md:table before:md:text-base before:md:not-italic before:md:normal-nums before:md:font-normal before:md:tracking-[normal] before:md:leading-6 before:md:list-outside before:md:list-disc before:md:outline-[3px] before:md:overscroll-x-auto before:md:overscroll-y-auto before:md:pointer-events-auto before:md:snap-align-none before:md:snap-normal before:md:snap-none before:md:no-underline before:md:decoration-auto before:md:indent-[0px] before:md:normal-case before:md:underline-offset-auto before:md:visible before:md:scroll-m-0 before:md:scroll-p-[auto] before:md:border-separate before:md:font-apple_system after:md:accent-auto after:md:aspect-auto after:md:box-border after:md:caret-transparent after:md:clear-both after:md:text-neutral-800 after:md:table after:md:text-base after:md:not-italic after:md:normal-nums after:md:font-normal after:md:tracking-[normal] after:md:leading-6 after:md:list-outside after:md:list-disc after:md:outline-[3px] after:md:overscroll-x-auto after:md:overscroll-y-auto after:md:pointer-events-auto after:md:snap-align-none after:md:snap-normal after:md:snap-none after:md:no-underline after:md:decoration-auto after:md:indent-[0px] after:md:normal-case after:md:underline-offset-auto after:md:visible after:md:scroll-m-0 after:md:scroll-p-[auto] after:md:border-separate after:md:font-apple_system">
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[85px] p-[5px] md:w-[127px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974932872_1.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[85px] p-[5px] md:w-[127px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974926450_2.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[85px] p-[5px] md:w-[127px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974920398_3.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                            <div className="box-border caret-transparent float-left min-h-px outline-[3px] w-[85px] p-[5px] md:w-[127px]">
                              <img
                                src="/lp-images-files-videos-fonts/images/rejuvacare/1726974912954_4.webp"
                                title=""
                                alt=""
                                className="box-border caret-transparent max-w-full outline-[3px] w-full"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-stretch box-border caret-transparent hidden flex-wrap justify-start outline-[3px] md:flex-nowrap"></div>
              <div className="items-stretch box-border caret-transparent hidden flex-wrap justify-start outline-[3px] md:flex-nowrap"></div>
              <div className="items-center box-border caret-transparent flex-col min-h-0 min-w-0 outline-[3px] w-full pt-3 px-[15px] md:min-h-[auto] md:min-w-[auto] md:px-2.5">
                <div className="text-orange-700 text-xl font-extrabold box-border caret-transparent leading-7 outline-[3px] text-left p-px font-montserrat">
                  <div className="text-base font-bold box-border caret-transparent leading-5 outline-[3px] md:text-xl md:font-extrabold md:leading-7">
                    TODAY ONLY SPECIAL OFFER
                  </div>
                </div>
                <div className="text-black text-[33px] font-bold box-border caret-transparent leading-[42px] outline-[3px] text-left p-px font-montserrat">
                  <div className="text-[23px] box-border caret-transparent leading-[33px] outline-[3px] md:text-[33px] md:leading-[42px]">
                    RejuvaKnee Massager
                  </div>
                </div>
                <div className="items-center box-border caret-transparent flex outline-[3px] w-full mt-2.5"></div>
                <div className="items-center box-border caret-transparent flex justify-between outline-[3px] w-full">
                  <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-[250px] md:w-[76%]">
                    <div className="text-xs box-border caret-transparent leading-[18px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:text-base md:leading-6">
                      Excellent
                    </div>
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722845257974_excellent_rating.svg"
                      alt=""
                      className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[84px] ml-3 md:w-[150px]"
                    />
                  </div>
                  <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-[300px] pl-2.5 md:w-full md:pl-0">
                    <div className="text-xs box-border caret-transparent leading-[18px] min-h-[auto] min-w-[auto] outline-[3px] text-left ml-2.5 p-px font-montserrat md:text-base md:leading-6 md:ml-5">
                      4,267 reviews on
                    </div>
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722845332292_trustpilot.svg"
                      alt=""
                      className="text-black items-center box-border caret-transparent h-5 justify-start max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[70px] ml-[5px] md:w-20"
                    />
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] text-left mt-3 font-montserrat">
                  RejuvaKnee’s “Triple Method” Massager Uses Advanced Therapy
                  Technology to Promote Blood Circulation in Your Knees and
                  Relieve Pain, Stiffness, And Muscle Tension Without Invasive
                  Surgery Or Painkillers
                  <div className="box-border caret-transparent outline-[3px]">
                    <br className="box-border caret-transparent outline-[3px]" />
                  </div>
                  <div className="box-border caret-transparent outline-[3px]">
                    <b className="font-bold box-border caret-transparent outline-[3px]">
                      Save more when you buy multiple units
                    </b>
                  </div>
                </div>
                <div className="bg-stone-100 box-border caret-transparent outline-[3px] w-full mt-5 rounded-[10px]">
                  <div className="items-center bg-emerald-500 box-border caret-transparent flex flex-col justify-between outline-[3px] w-full pl-5 py-3 rounded-t-[10px] md:flex-row">
                    <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full md:w-[90%]">
                      <div className="text-white font-bold box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:leading-6">
                        2x RejuvaKnee Massagers
                      </div>
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722845784568_star.svg"
                        alt=""
                        className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-5 ml-[7px]"
                      />
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full pl-px md:justify-end md:pl-2.5">
                      <div className="text-white box-border caret-transparent block leading-5 min-h-[auto] min-w-[auto] outline-[3px] line-through md:leading-6">
                        <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          $239.99
                        </span>
                      </div>
                      <div className="text-white box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] ml-2.5 mr-[5px] py-px font-montserrat md:leading-6">
                        <b className="font-bold box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                            $67.50
                          </span>
                        </b>
                        each
                      </div>
                      <div className="bg-black box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[100px] ml-1.5 px-2.5 py-0.5 rounded-[20px]">
                        <div className="text-white text-[13px] font-bold box-border caret-transparent leading-4 outline-[3px] p-px font-montserrat md:text-sm md:leading-[21px]">
                          40% OFF
                        </div>
                      </div>
                    </div>
                    <div className="text-white box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-[11px] p-px font-montserrat"></div>
                  </div>
                  <div className="box-border caret-transparent outline-[3px] w-full border-neutral-300 pt-3 pb-[15px] px-5 rounded-b-[10px] border-b border-l border-r border-solid">
                    <div className="items-start box-border caret-transparent flex justify-start outline-[3px] text-left w-full mt-2.5 md:items-center">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-left w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat">
                        <div className="box-border caret-transparent leading-6 outline-[3px] text-left md:leading-5">
                          <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                            2 RejuvaKnee 
                          </b>
                          Triple Method Massager Treatment Kits
                        </div>
                        <div className="box-border caret-transparent outline-[3px] text-left"></div>
                      </div>
                    </div>
                    <div className="items-start box-border caret-transparent flex justify-start outline-[3px] text-left w-full mt-2.5 md:items-center">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-left w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat">
                        <div className="box-border caret-transparent leading-6 outline-[3px] text-left md:leading-5">
                          <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                            Free bonus gifts
                          </b>
                          including Arthritis Relief Nutrition Plan
                        </div>
                        <div className="box-border caret-transparent outline-[3px] text-left"></div>
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-left">
                          <span className="box-border caret-transparent outline-[3px]">
                            90-Day Money Back Guarantee
                          </span>
                        </div>
                        <div className="box-border caret-transparent outline-[3px]"></div>
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                        <div className="box-border caret-transparent outline-[3px] text-left">
                          Free tracked &amp; insured shipping
                        </div>
                        <div className="box-border caret-transparent outline-[3px]"></div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-stone-100 box-border caret-transparent outline-[3px] w-full mt-5 rounded-[10px]">
                  <div className="items-center bg-zinc-100 box-border caret-transparent flex flex-col justify-between outline-[3px] w-full border border-neutral-300 pl-5 py-3 rounded-t-[10px] border-solid md:flex-row">
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full md:w-[90%]">
                      <div className="text-zinc-800 font-bold box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:leading-6">
                        1x RejuvaKnee Massagers
                      </div>
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722845784568_star.svg"
                        alt=""
                        className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-5 ml-[7px]"
                      />
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full md:justify-end">
                      <div className="box-border caret-transparent block leading-5 min-h-[auto] min-w-[auto] outline-[3px] line-through md:leading-6">
                        <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          $119.99
                        </span>
                      </div>
                      <div className="text-zinc-800 box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] ml-2 mr-[5px] p-px font-montserrat md:leading-6">
                        <b className="font-bold box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                            $79.99
                          </span>
                        </b>
                        each
                      </div>
                      <div className="bg-black box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[100px] ml-1.5 px-2.5 py-0.5 rounded-[20px]">
                        <div className="text-white text-[13px] font-bold box-border caret-transparent leading-[19.5px] outline-[3px] p-px font-montserrat md:text-sm md:leading-[21px]">
                          33% OFF
                        </div>
                      </div>
                    </div>
                    <div className="text-zinc-800 box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-[11px] p-px font-montserrat"></div>
                  </div>
                  <div className="box-border caret-transparent outline-[3px] w-full border-neutral-300 pt-3 pb-[15px] px-5 rounded-b-[10px] border-b border-l border-r border-solid">
                    <div className="items-start box-border caret-transparent flex justify-start outline-[3px] text-left w-full mt-2.5 md:items-center">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-left w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat">
                        <div className="box-border caret-transparent leading-6 outline-[3px] text-left md:leading-5">
                          <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                            1 RejuvaKnee 
                          </b>
                          Triple Method Massager Treatment Kit
                        </div>
                        <div className="box-border caret-transparent outline-[3px] text-left"></div>
                      </div>
                    </div>
                    <div className="box-border caret-transparent hidden outline-[3px] w-full mt-2.5 pb-2.5">
                      <div className="items-start box-border caret-transparent flex justify-start outline-[3px] text-left w-full mt-2.5 md:items-center">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] text-left w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] text-left p-px font-montserrat">
                          <div className="box-border caret-transparent leading-6 outline-[3px] text-left md:leading-5">
                            <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                              Free bonus gifts
                            </b>
                            <span className="box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                              including Arthritis Relief Nutrition Plan
                            </span>
                          </div>
                          <div className="box-border caret-transparent outline-[3px] text-left"></div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] p-px font-montserrat">
                          <div className="box-border caret-transparent outline-[3px]">
                            <span className="box-border caret-transparent outline-[3px]">
                              90-Day Money Back Guarantee
                              <br className="box-border caret-transparent outline-[3px]" />
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] p-px font-montserrat">
                          <div className="box-border caret-transparent outline-[3px]">
                            <span className="box-border caret-transparent outline-[3px] text-left">
                              Free tracked &amp; insured shipping
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-[5px]">
                      <a
                        title="View All"
                        className="text-zinc-800 box-border caret-transparent block max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-center underline px-2.5 font-helvetica"
                      >
                        View All
                      </a>
                    </div>
                  </div>
                </div>
                <div className="bg-stone-100 box-border caret-transparent outline-[3px] w-full mt-5 rounded-[10px]">
                  <div className="items-center bg-zinc-100 box-border caret-transparent flex flex-col justify-between outline-[3px] w-full border border-neutral-300 pl-5 py-3 rounded-t-[10px] border-solid md:flex-row">
                    <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full md:w-[90%]">
                      <div className="text-zinc-800 font-bold box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:leading-6">
                        4x RejuvaKnee Massagers
                      </div>
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722845784568_star.svg"
                        alt=""
                        className="text-black box-border caret-transparent h-[18px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-5 ml-[7px]"
                      />
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full md:justify-end">
                      <div className="box-border caret-transparent block leading-5 min-h-[auto] min-w-[auto] outline-[3px] line-through md:leading-6">
                        <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          $479.99
                        </span>
                      </div>
                      <div className="text-zinc-800 box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] ml-2 mr-[5px] p-px font-montserrat md:leading-6">
                        <b className="font-bold box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                          <span className="box-border caret-transparent leading-5 outline-[3px] md:leading-6">
                            $59.99
                          </span>
                        </b>
                        each
                      </div>
                      <div className="bg-black box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-[100px] ml-1.5 px-2.5 py-0.5 rounded-[20px]">
                        <div className="text-white text-[13px] font-bold box-border caret-transparent leading-4 outline-[3px] p-px font-montserrat md:text-sm md:leading-[21px]">
                          50% OFF
                        </div>
                      </div>
                    </div>
                    <div className="text-zinc-800 box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-[11px] p-px font-montserrat"></div>
                  </div>
                  <div className="box-border caret-transparent outline-[3px] w-full border-neutral-300 pt-3 pb-[15px] px-5 rounded-b-[10px] border-b border-l border-r border-solid">
                    <div className="items-start box-border caret-transparent flex justify-start outline-[3px] text-left w-full mt-2.5 md:items-center">
                      <img
                        title=""
                        src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                        alt=""
                        className="text-black box-border caret-transparent h-[25px] max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-left w-[25px] ml-px mr-2.5"
                      />
                      <div className="box-border caret-transparent leading-5 min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat">
                        <div className="box-border caret-transparent leading-6 outline-[3px] text-left md:leading-5">
                          <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                            4 RejuvaKnee 
                          </b>
                          Triple Method Massager Treatment Kits
                        </div>
                        <div className="box-border caret-transparent outline-[3px] text-left"></div>
                      </div>
                    </div>
                    <div className="box-border caret-transparent hidden outline-[3px] w-full mt-2.5 pb-2.5">
                      <div className="items-start box-border caret-transparent flex justify-start outline-[3px] w-full">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] text-left p-px font-montserrat">
                          <div className="box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                            <span className="box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                              <b className="font-bold box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                                Free bonus gifts 
                              </b>
                               including Arthritis Relief Nutrition Plan + a{" "}
                              <u className="box-border caret-transparent leading-6 outline-[3px] underline md:leading-5">
                                FREE{" "}
                                <span className="box-border caret-transparent leading-6 outline-[3px] md:leading-5">
                                  $20
                                </span>
                                gift card
                              </u>
                              for your next order
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] p-px font-montserrat">
                          <div className="box-border caret-transparent outline-[3px]">
                            <span className="box-border caret-transparent outline-[3px]">
                              90-Day Money Back Guarantee
                              <br className="box-border caret-transparent outline-[3px]" />
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-2.5">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722846383829_icon_check.webp"
                          alt=""
                          className="text-black box-border caret-transparent h-[25px] max-w-full outline-[3px] w-[25px] ml-px mr-2.5"
                        />
                        <div className="box-border caret-transparent leading-5 outline-[3px] p-px font-montserrat">
                          <div className="box-border caret-transparent outline-[3px]">
                            <span className="box-border caret-transparent outline-[3px] text-left">
                              Free tracked &amp; insured shipping
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="items-center box-border caret-transparent flex justify-start outline-[3px] w-full mt-[5px]">
                      <a
                        title="View All"
                        className="text-zinc-800 box-border caret-transparent block max-w-full min-h-[auto] min-w-[auto] outline-[3px] text-center underline px-2.5 font-helvetica"
                      >
                        View All
                      </a>
                    </div>
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] w-full mt-5">
                  <a
                    title="Get up to 50% off today only  "
                    className="text-white text-[17px] font-extrabold self-center bg-emerald-500 shadow-[rgba(0,0,0,0.19)_0px_2px_7px_1px] box-border caret-transparent flex justify-center leading-[23px] max-w-full outline-[3px] text-center w-full px-2.5 py-5 rounded-[10px] font-montserrat md:text-[19px] md:leading-[23.16px]"
                  >
                    Continue to select your bundle  ➜
                  </a>
                </div>
                <div className="items-end bg-neutral-100 box-border caret-transparent flex justify-center outline-[3px] w-full mt-5 pl-[5px] pr-0 pt-px rounded-[5px] md:w-auto md:pt-0 md:px-2.5">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full mb-px pl-2.5 py-2.5 md:mb-0 md:pt-0 md:pb-[7px]">
                    <div className="text-neutral-900 font-bold box-border caret-transparent leading-6 outline-[3px] text-left mt-px p-px font-montserrat md:leading-[19.2px]">
                      Doctor Reviewed
                    </div>
                    <div className="text-neutral-900 text-sm font-medium box-border caret-transparent leading-[21px] outline-[3px] text-left mt-2 mb-2.5 p-px font-montserrat md:text-base md:font-normal md:leading-6">
                      &quot;As a knee pain expert, I only want the very best for
                      my patients - therefore, I trust and recommend
                      RejuvaKnee.&quot;
                    </div>
                    <div className="text-neutral-900 text-sm font-medium box-border caret-transparent leading-[21px] outline-[3px] text-left mt-2 mb-2.5 p-px font-montserrat md:text-base md:font-normal md:leading-6">
                      <i className="text-[13px] italic font-medium box-border caret-transparent leading-[21px] outline-[3px] md:text-base md:font-normal md:leading-6">
                        Dr. James Barkley, Joint Health expert
                      </i>
                    </div>
                  </div>
                  <div className="items-center bg-[url('/lp-images-files-videos-fonts/images/rejuvacare/1722821367050_dr_james_barkley.webp"></div>
                </div>
                <div className="box-border caret-transparent outline-[3px] w-full mt-5">
                  <img
                    title=""
                    src="/lp-images-files-videos-fonts/images/rejuvacare/1722821080411_cc_payment_options.webp"
                    alt=""
                    className="text-black box-border caret-transparent inline max-w-full outline-[3px] w-full"
                  />
                </div>
                <div className="items-start box-border caret-transparent flex justify-start outline-[3px] w-full px-2.5 py-3 md:items-center md:justify-center">
                  <img
                    title=""
                    src="/lp-images-files-videos-fonts/images/rejuvacare/1722847956492_lock.svg"
                    alt=""
                    className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-[15px] mr-2.5 mt-0.5 md:mt-0"
                  />
                  <div className="box-border caret-transparent leading-[19px] min-h-[auto] min-w-[auto] outline-[3px] text-left p-px font-montserrat md:leading-6">
                    All transactions are secure and encrypted
                  </div>
                </div>
                <div className="items-start bg-neutral-100 box-border caret-transparent flex flex-col justify-center outline-[3px] w-full px-[15px] py-3 rounded-[15px]">
                  <div className="items-center box-border caret-transparent flex justify-around min-h-[auto] min-w-[auto] outline-[3px] w-full">
                    <div className="items-center box-border caret-transparent flex justify-start min-h-[auto] min-w-[auto] outline-[3px] w-full"></div>
                  </div>
                  <div className="items-center box-border caret-transparent flex justify-around min-h-[auto] min-w-[auto] outline-[3px] w-full"></div>
                  <div className="box-border caret-transparent flex flex-col justify-between min-h-[auto] min-w-[auto] outline-[3px] w-full py-3 md:flex-row">
                    <div className="items-start box-border caret-transparent flex justify-between min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="items-center box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-3">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722848377355_line_icon_free_shipping.webp"
                          alt=""
                          className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-10"
                        />
                        <div className="text-xs box-border caret-transparent leading-4 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                          Free Shipping
                          <div className="box-border caret-transparent outline-[3px]">
                            <br className="box-border caret-transparent outline-[3px]" />
                          </div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-3">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722848371698_line_icon_90_day_guarantee.webp"
                          alt=""
                          className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-10"
                        />
                        <div className="text-xs box-border caret-transparent leading-4 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                          90-Day Money Back Guarantee
                        </div>
                      </div>
                    </div>
                    <div className="items-stretch box-border caret-transparent flex justify-between min-h-[auto] min-w-[auto] outline-[3px] w-full">
                      <div className="items-center box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-3">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722848366371_line_icon_customer_service.webp"
                          alt=""
                          className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-10"
                        />
                        <div className="text-xs box-border caret-transparent leading-4 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                          24/7 Customer Service
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent flex flex-col justify-center min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-3">
                        <img
                          title=""
                          src="/lp-images-files-videos-fonts/images/rejuvacare/1722848354019_line_icon_100k_happy_customers.webp"
                          alt=""
                          className="text-black box-border caret-transparent max-w-full min-h-[auto] min-w-[auto] outline-[3px] w-10"
                        />
                        <div className="text-xs box-border caret-transparent leading-4 min-h-[auto] min-w-[auto] outline-[3px] p-px font-montserrat">
                          Over 100,000 Happy Customers
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-3">
                    <div className="box-border caret-transparent outline-[3px] p-px font-montserrat">
                      <b className="font-bold box-border caret-transparent outline-[3px]">
                        No Risk. No Pressure.
                      </b>
                    </div>
                  </div>
                </div>
                <div className="bg-stone-100 box-border caret-transparent outline-[3px] w-full mt-5 rounded-[10px]"></div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-center bg-white box-border caret-transparent flex basis-px flex-col-reverse grow justify-start max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pb-[35px] px-[5px] md:flex-row md:justify-center md:px-[100px]">
              <div className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full mt-2.5 mx-px pt-px pb-0 px-[15px] md:w-[94%] md:mt-0 md:mx-0 md:pl-2.5 md:pr-[30px] md:py-3">
                <div className="text-[23px] font-bold box-border caret-transparent hidden leading-[33px] min-h-0 min-w-0 outline-[3px] text-left pr-[30px] font-montserrat md:text-[33px] md:block md:leading-[46.2px] md:min-h-[auto] md:min-w-[auto] md:pr-10">
                  Approved &amp; Endorsed by US Medical Professionals
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-left mt-2.5 pr-px font-montserrat md:mt-6 md:pr-10">
                  Dr. James Barkely has studied and clinically proven the
                  incredible benefits of the RejuvaKnee “Triple Method”
                  Massager.
                  <div className="box-border caret-transparent outline-[3px]">
                    <br className="box-border caret-transparent outline-[3px]" />
                    <div className="box-border caret-transparent outline-[3px]">
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="box-border caret-transparent outline-[3px]"></div>
                      <div className="box-border caret-transparent outline-[3px]">
                        Through his dedicated research, he has determined the
                        massager to be an effective and safe form of
                        rehabilitative care.
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-stretch box-border caret-transparent flex flex-col justify-between min-h-[auto] min-w-[auto] outline-[3px] w-full mt-px mx-px md:items-center md:justify-end md:mt-0 md:mx-0">
                <div className="text-[23px] font-bold box-border caret-transparent block leading-[33px] min-h-[auto] min-w-[auto] outline-[3px] text-left mb-5 pl-[15px] pr-[30px] font-montserrat md:text-[33px] md:hidden md:leading-[46.2px] md:min-h-0 md:min-w-0 md:mb-0 md:pl-0 md:pr-10">
                  Approved &amp; Endorsed by US Medical Professionals
                </div>
                <div className="items-start box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-0 md:py-3">
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-0 md:px-2.5 md:py-3">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722924582232_endorser_1.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline h-full max-w-full outline-[3px] w-[95%] mb-px md:w-full md:mb-0"
                    />
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full p-0 md:px-2.5 md:py-3">
                    <img
                      title=""
                      src="/lp-images-files-videos-fonts/images/rejuvacare/1722924562849_endorser_2.webp"
                      alt=""
                      className="text-black box-border caret-transparent inline h-auto max-w-full outline-[3px] w-[95%] md:h-full md:w-full"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-start max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative items-start bg-sky-100 box-border caret-transparent basis-px grow justify-start max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-[30px] pb-5 px-[15px] md:items-center md:justify-center md:pt-[50px] md:pb-10 md:px-[100px]">
              <div className="box-border caret-transparent outline-[3px] w-full px-0 md:px-2.5">
                <div className="text-[23px] font-bold box-border caret-transparent leading-[33px] outline-[3px] text-center px-2.5 font-montserrat md:text-[33px] md:leading-[46.2px] md:px-0">
                  3 Simple Steps To Relieve Knee Pain At Home
                </div>
              </div>
              <div className="items-center box-border caret-transparent flex flex-col justify-around outline-[3px] w-full pt-0 px-0 md:items-start md:flex-row md:pt-3 md:px-2.5">
                <div className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-auto mx-px pt-0 pb-3 px-0 md:w-full md:mx-0 md:pt-3 md:px-5">
                  <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                    Step 1
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/0a5c5afde7a84890b9c720405d2d4c69.mp4"
                      poster=""
                      className="box-border caret-transparent inline max-w-full outline-[3px] align-baseline mt-2.5 rounded-[15px]"
                    ></video>
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat">
                    Turn your device on and select the heat and massage
                    intensity you desire
                  </div>
                </div>
                <div className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-auto mx-px pt-0 pb-3 px-0 md:w-full md:mx-0 md:pt-3 md:px-5">
                  <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                    Step 2
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/028808b4b66741908bb0440183ebf397.mp4"
                      className="box-border caret-transparent inline max-w-full outline-[3px] align-baseline mt-2.5 rounded-[15px]"
                    ></video>
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat">
                    Allow RejuvaKnee to work its magic for 15-30 minutes
                  </div>
                </div>
                <div className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-auto mx-px pt-0 pb-3 px-0 md:w-full md:mx-0 md:pt-3 md:px-5">
                  <div className="text-lg font-bold box-border caret-transparent leading-[22px] min-h-[auto] min-w-[auto] outline-[3px] mt-5 p-px font-montserrat md:leading-[27px]">
                    Step 3
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                    <video
                      loop
                      autoPlay
                      playsInline
                      muted
                      src="https://cdn.shopify.com/videos/c/o/v/8102ace2a7714cb1aa9f83ee2598582f.mp4"
                      className="box-border caret-transparent inline max-w-full outline-[3px] align-baseline mt-2.5 rounded-[15px]"
                    ></video>
                  </div>
                  <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] mt-2.5 p-px font-montserrat">
                    Enjoy pain-free knees and get back to living your life to
                    its fullest!
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center self-center bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-white box-border caret-transparent basis-full grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto px-[15px] py-5 md:basis-0 md:pt-[50px] md:pb-[30px] md:px-[100px]">
              <div className="text-neutral-900 text-[23px] font-bold box-border caret-transparent leading-[30px] outline-[3px] mt-2.5 pt-px pb-2.5 px-0 font-montserrat md:text-[33px] md:leading-[42px] md:mt-0 md:pt-2.5 md:px-2.5">
                Below is a list of frequently asked 
                <span className="text-[23px] box-border caret-transparent leading-[30px] outline-[3px] text-left md:text-[33px] md:leading-[42px]">
                  questions about RejuvaKnee
                </span>
              </div>
              <div className="box-border caret-transparent outline-[3px] pt-[5px] pb-px md:pb-[5px]">
                <div className="box-border caret-transparent outline-[3px] mx-px my-2.5 px-px py-[5px]">
                  <div className="box-border caret-transparent outline-[3px]">
                    <details className="box-border caret-transparent outline-[3px]">
                      <summary className="bg-white box-border caret-transparent list-item list-inside list-[disclosure-open] outline-[3px] rounded-[5px]">
                        <div className="items-center box-border caret-transparent flex outline-[3px] border-zinc-300 p-[5px] border-b border-solid">
                          <div className="text-[19px] box-border caret-transparent basis-[0%] grow-[9.9] leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left pl-[5px] font-montserrat md:text-lg">
                            Will RejuvaKnee work for my Knee Pain and Stiffness?
                          </div>
                          <div className="box-border caret-transparent basis-[0%] grow-[0.1] min-h-[auto] min-w-[auto] outline-[3px] p-[5px]">
                            <div className="box-border caret-transparent hidden outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-open] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                            <div className="box-border caret-transparent outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-open] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                          </div>
                        </div>
                      </summary>
                      <div className="box-border caret-transparent outline-[3px] text-left px-2.5 py-[5px] rounded-b-[5px]">
                        <div className="text-neutral-900 box-border caret-transparent outline-[3px] mt-2.5 pr-0 font-montserrat md:pr-40">
                          Absolutely! RejuvaKnee’s “Triple Method” Massager is
                          your ticket to relief from knee pain. Thanks to its
                          innovative technology, it combines Heat, Massage, and
                          Compression Therapy, designed specifically for your
                          knees. Say goodbye to discomfort and hello to soothing
                          relief!
                        </div>
                      </div>
                    </details>
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] mx-px my-2.5 px-px py-[5px]">
                  <div className="box-border caret-transparent outline-[3px]">
                    <details className="box-border caret-transparent outline-[3px]">
                      <summary className="bg-white box-border caret-transparent list-item list-inside list-[disclosure-closed] outline-[3px] rounded-[5px]">
                        <div className="items-center box-border caret-transparent flex outline-[3px] border-zinc-300 p-[5px] border-b border-solid">
                          <div className="text-[19px] box-border caret-transparent basis-[0%] grow-[9.9] leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left pl-[5px] font-montserrat md:text-lg">
                            How soon will I notice results with RejuvaKnee?
                          </div>
                          <div className="box-border caret-transparent basis-[0%] grow-[0.1] min-h-[auto] min-w-[auto] outline-[3px] p-[5px]">
                            <div className="box-border caret-transparent outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                            <div className="box-border caret-transparent hidden outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                          </div>
                        </div>
                      </summary>
                      <div className="box-border caret-transparent outline-[3px] text-left px-2.5 py-[5px]">
                        <div className="text-neutral-900 box-border caret-transparent outline-[3px] mt-2.5 pr-0 font-montserrat md:pr-40">
                          Get ready for rapid relief! Many users have reported
                          significant improvements after just one session. For
                          lasting benefits, regular use is the key. RejuvaKnee
                          ensures continuous comfort and relief.
                        </div>
                      </div>
                    </details>
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] mx-px my-2.5 px-px py-[5px]">
                  <div className="box-border caret-transparent outline-[3px]">
                    <details className="box-border caret-transparent outline-[3px]">
                      <summary className="bg-white box-border caret-transparent list-item list-inside list-[disclosure-closed] outline-[3px] rounded-[5px]">
                        <div className="items-center box-border caret-transparent flex outline-[3px] border-zinc-300 p-[5px] border-b border-solid">
                          <div className="text-[19px] box-border caret-transparent basis-[0%] grow-[9.9] leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left pl-[5px] font-montserrat md:text-lg">
                            Whats the ideal duration for each RejuvaKnee therapy
                            session?
                          </div>
                          <div className="box-border caret-transparent basis-[0%] grow-[0.1] min-h-[auto] min-w-[auto] outline-[3px] p-[5px]">
                            <div className="box-border caret-transparent outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                            <div className="box-border caret-transparent hidden outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                          </div>
                        </div>
                      </summary>
                      <div className="box-border caret-transparent outline-[3px] text-left px-2.5 py-[5px]">
                        <div className="text-neutral-900 box-border caret-transparent outline-[3px] mt-2.5 pr-0 font-montserrat md:pr-40">
                          No need to rush – we&#39;ve got your comfort covered.
                          A mere 15-30 minutes a day with RejuvaKnee’s “Triple
                          Method” Massager is all it takes. It&#39;s like having
                          a personal knee therapist, ready to pamper you
                          whenever you need it.
                        </div>
                      </div>
                    </details>
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] mx-px my-2.5 px-px py-[5px]">
                  <div className="box-border caret-transparent outline-[3px]">
                    <details className="box-border caret-transparent outline-[3px]">
                      <summary className="bg-white box-border caret-transparent list-item list-inside list-[disclosure-closed] outline-[3px] rounded-[5px]">
                        <div className="items-center box-border caret-transparent flex outline-[3px] border-zinc-300 p-[5px] border-b border-solid">
                          <div className="text-[19px] box-border caret-transparent basis-[0%] grow-[9.9] leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left pl-[5px] font-montserrat md:text-lg">
                            What is RejuvaKnee does not meet my expectation?
                          </div>
                          <div className="box-border caret-transparent basis-[0%] grow-[0.1] min-h-[auto] min-w-[auto] outline-[3px] p-[5px]">
                            <div className="box-border caret-transparent outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                            <div className="box-border caret-transparent hidden outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                          </div>
                        </div>
                      </summary>
                      <div className="box-border caret-transparent outline-[3px] text-left px-2.5 py-[5px]">
                        <div className="text-neutral-900 box-border caret-transparent outline-[3px] mt-2.5 pr-0 font-montserrat md:pr-40">
                          Rest easy with our &#39;Satisfaction Guaranteed&#39;
                          promise! If RejuvaKnee doesn&#39;t exceed your
                          expectations, we&#39;re here to make it right.
                          We&#39;re committed to ensuring your journey to
                          comfort is smooth, risk-free, and delightful.
                        </div>
                      </div>
                    </details>
                  </div>
                </div>
                <div className="box-border caret-transparent outline-[3px] mx-px my-2.5 px-px py-[5px]">
                  <div className="box-border caret-transparent outline-[3px]">
                    <details className="box-border caret-transparent outline-[3px]">
                      <summary className="bg-white box-border caret-transparent list-item list-inside list-[disclosure-closed] outline-[3px] rounded-[5px]">
                        <div className="items-center box-border caret-transparent flex outline-[3px] border-zinc-300 p-[5px] border-b border-solid">
                          <div className="text-[19px] box-border caret-transparent basis-[0%] grow-[9.9] leading-[28.5px] min-h-[auto] min-w-[auto] outline-[3px] text-left pl-[5px] font-montserrat md:text-lg">
                            Could you tell me about your return policy?
                          </div>
                          <div className="box-border caret-transparent basis-[0%] grow-[0.1] min-h-[auto] min-w-[auto] outline-[3px] p-[5px]">
                            <div className="box-border caret-transparent outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                            <div className="box-border caret-transparent hidden outline-[3px] px-[5px]">
                              <i className="font-black box-border caret-transparent inline-block leading-4 outline-[3px] font-font_awesome_5_free before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:text-base before:not-italic before:normal-nums before:font-black before:tracking-[normal] before:leading-4 before:list-inside before:list-[disclosure-closed] before:outline-[3px] before:pointer-events-auto before:no-underline before:indent-[0px] before:normal-case before:visible before:border-separate before:font-font_awesome_5_free"></i>
                            </div>
                          </div>
                        </div>
                      </summary>
                      <div className="box-border caret-transparent outline-[3px] text-left px-2.5 py-[5px]">
                        <div className="text-neutral-900 box-border caret-transparent outline-[3px] mt-2.5 pr-0 font-montserrat md:pr-40">
                          We&#39;re dedicated to your satisfaction, and that
                          includes a hassle-free return policy. If RejuvaKnee
                          doesn&#39;t leave you completely satisfied within 90
                          days, simply return it for a stress-free refund. Your
                          peace of mind is our priority!
                        </div>
                      </div>
                    </details>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-stretch bg-neutral-100 box-border caret-transparent flex flex-wrap justify-center max-w-full outline-[3px] md:flex-nowrap">
            <div className="relative bg-stone-100 box-border caret-transparent basis-full flex-col grow max-w-[1400px] min-h-[25px] outline-[3px] w-min m-auto pt-5 pb-[100px] px-[15px] md:basis-0 md:pb-5 md:px-[100px]">
              <div className="text-zinc-800 text-sm box-border caret-transparent leading-[21px] outline-[3px] mb-2.5 px-0 py-2.5 font-montserrat md:mb-0 md:px-2.5">
                <b className="font-bold box-border caret-transparent outline-[3px]">
                  MEDICAL &amp; HEALTH DISCLAIMER:
                </b>
                The information and other content provided in this page, or in
                any linked materials, are not intended and should not be
                construed as medical advice, nor is the information a substitute
                for professional medical expertise or treatment.
                <div className="box-border caret-transparent outline-[3px]">
                  <br className="box-border caret-transparent outline-[3px]" />
                  <div className="box-border caret-transparent outline-[3px]"></div>
                  <div className="box-border caret-transparent outline-[3px]"></div>
                  <div className="box-border caret-transparent outline-[3px]"></div>
                  <div className="box-border caret-transparent outline-[3px]">
                    If you or any other person has a medical concern, you should
                    consult with your health care provider or seek other
                    professional medical treatment. Never disregard professional
                    medical advice or delay in seeking it because of something
                    that have read on this page or in any linked materials. If
                    you think you may have a medical emergency, call your doctor
                    or emergency services immediately.
                  </div>
                </div>
              </div>
              <div className="text-xs box-border caret-transparent leading-4 outline-[3px] mb-2.5 font-montserrat">
                © RejuvaKnee 2024. All Rights Reserved
              </div>
              <div className="text-zinc-800 text-xs box-border caret-transparent leading-[18px] outline-[3px] pt-px pb-2.5 px-2.5 font-montserrat">
                Terms &amp; Conditions | Privacy Policy
                <div className="box-border caret-transparent outline-[3px]">
                  +1 302-261-9613 | support@myrejuvaknee.com
                </div>
              </div>
            </div>
          </div>
          <div className="fixed items-stretch self-center bg-white box-border caret-transparent hidden flex-row-reverse flex-wrap justify-start max-w-full outline-[3px] w-full z-[99999] border-neutral-200 px-2.5 py-0 border-t-2 border-solid bottom-0 md:sticky md:flex-row md:flex-nowrap md:py-2.5">
            <div className="relative box-border caret-transparent basis-full grow max-w-[1200px] min-h-[25px] outline-[3px] w-min mx-auto pt-[5px] pb-2.5 px-px md:basis-0 md:py-px">
              <div className="box-border caret-transparent outline-[3px] w-full p-px">
                <div className="box-border caret-transparent flex justify-center outline-[3px] w-full mt-px p-px">
                  <a
                    title="Get up to 50% off today only  "
                    className="text-white text-xl font-extrabold self-center bg-emerald-500 shadow-[rgba(0,0,0,0.19)_0px_2px_7px_1px] box-border caret-transparent flex justify-center leading-[23px] max-w-full outline-[3px] text-center w-full px-2.5 py-5 rounded-[10px] font-montserrat md:text-[19px] md:leading-[23.16px] md:w-[367px]"
                  >
                    Get 50% off today only  ➜
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
