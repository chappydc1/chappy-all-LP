import { AmalaComparisonRow } from "../AmalaComparisonSection/components/AmalaComparisonRow";
import { AmalaCtaBanner } from "../../components/AmalaCtaBanner";

export const AmalaComparisonSection = () => {
  return (
    <section
      className="relative items-center bg-gray-100 bg-no-repeat box-border caret-transparent flex flex-col justify-items-center min-h-[auto] min-w-[auto] outline-[3px] w-full bg-center px-[15px] py-[45px] md:pt-[60px] md:pb-20"
    >
      <div
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1170px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center md:flex-row"
      >
        <div
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              className="text-black text-[27.0583px] box-border caret-transparent leading-[32.47px] max-w-[800px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-[35px] md:leading-[42px]"
            >
              <div
                className="text-[27.0583px] box-border caret-transparent leading-[32.47px] outline-[3px] w-full md:text-[35px] md:leading-[42px]"
              >
                <p
                  className="text-[27.0583px] box-border caret-transparent leading-[32.47px] outline-[3px] w-full md:text-[35px] md:leading-[42px]"
                >
                  <strong
                    className="text-[27.0583px] font-bold box-border caret-transparent leading-[32.47px] outline-[3px] md:text-[35px] md:leading-[42px]"
                  >
                    PrimeCell H2 VS Standard Antioxidant Supplements
                  </strong>
                </p>
              </div>
            </div>
          </div>
          <div
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <img
                src="/lp-images-files-videos-fonts/images/amala/18.svg"
                alt=""
                title=""
                className="absolute box-border caret-transparent max-w-full outline-[3px] w-full top-0 md:top-[-45px]"
              />
            </div>
          </div>
          <div
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              className="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <img
                src="/lp-images-files-videos-fonts/images/amala/19.svg"
                alt=""
                title=""
                className="absolute box-border caret-transparent max-w-full outline-[3px] w-full bottom-0 md:bottom-[-45px]"
              />
            </div>
          </div>
        </div>
      </div>
      <div
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1000px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center md:flex-row"
      >
        <div
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <div
                className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto] border-gray-800 mt-[100px] rounded-[10px] border-[3px] border-solid md:mt-[190px]"
              >
                <div
                  className="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                >
                  <div
                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <div
                        className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                      >
                        <div
                          className="box-border caret-transparent flex basis-[0%] flex-col grow h-20 min-h-[auto] outline-[3px] px-[5px] md:h-[100px] md:px-2.5"
                        >
                          <div
                            className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                          >
                            <div
                              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                            >
                              <div
                                className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                              >
                                <div
                                  className="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                                >
                                  <div
                                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                                  >
                                    <div
                                      className="items-center box-border caret-transparent flex flex-col mt-[-55px] min-h-[auto] min-w-[auto] outline-[3px] w-full md:mt-[-175px]"
                                    >
                                      <img
                                        src="/lp-images-files-videos-fonts/images/amala/1763722198516_PRIME_CELL_H2_hero_.webp"
                                        alt=""
                                        title=""
                                        className="aspect-square box-border caret-transparent h-full max-w-full min-h-[auto] min-w-[auto] object-scale-down outline-[3px] w-[225px] md:h-auto"
                                      />
                                    </div>
                                  </div>
                                  <div
                                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                                  >
                                    <div
                                      className="text-zinc-700 text-[13.0583px] font-semibold box-border caret-transparent leading-[15.67px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full mt-2.5 md:text-2xl md:leading-[28.8px]"
                                    >
                                      <div
                                        className="text-[13.0583px] box-border caret-transparent leading-[15.67px] outline-[3px] w-full md:text-2xl md:leading-[28.8px]"
                                      >
                                        <p
                                          className="text-gray-800 text-[13.0583px] box-border caret-transparent leading-[15.67px] outline-[3px] md:text-2xl md:leading-[28.8px]"
                                        >
                                          PrimeCell H2
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="relative bg-gray-800 box-border caret-transparent flex basis-[0%] flex-col grow h-[110px] mt-[-30px] max-w-[46%] min-h-[auto] min-w-[46%] outline-[3px] w-[46%] px-[5px] rounded-t-[10px] md:h-[155px] md:mt-[-55px] md:max-w-[40%] md:min-w-[40%] md:w-2/5 md:px-2.5"
                        >
                          <div
                            className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                          >
                            <div
                              className="relative text-zinc-700 text-[22.0583px] items-center bg-white box-border caret-transparent flex h-[54px] leading-[26.47px] min-h-[auto] min-w-[auto] outline-[3px] text-center top-[-26px] w-[54px] border-gray-800 rounded-[300px] border-[3px] border-solid md:text-[28px] md:h-[76px] md:leading-[33.6px] md:top-[-35px] md:w-[76px]"
                            >
                              <div
                                className="text-[22.0583px] box-border caret-transparent leading-[26.47px] min-h-[auto] min-w-[auto] outline-[3px] w-full md:text-[28px] md:leading-[33.6px]"
                              >
                                <p
                                  className="text-[22.0583px] box-border caret-transparent leading-[26.47px] outline-[3px] md:text-[28px] md:leading-[33.6px]"
                                >
                                  <strong
                                    className="text-gray-800 text-[22.0583px] font-bold box-border caret-transparent leading-[26.47px] outline-[3px] md:text-[28px] md:leading-[33.6px]"
                                  >
                                    VS
                                  </strong>
                                </p>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="box-border caret-transparent flex basis-[0%] flex-col grow h-20 min-h-[auto] outline-[3px] px-[5px] md:h-[100px] md:px-2.5"
                        >
                          <div
                            className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                          >
                            <div
                              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                            >
                              <div
                                className="items-start box-border caret-transparent flex flex-wrap min-h-[auto] min-w-[auto]"
                              >
                                <div
                                  className="box-border caret-transparent flex basis-[0%] flex-col grow min-h-[auto] outline-[3px]"
                                >
                                  <div
                                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                                  >
                                    <div
                                      className="items-center box-border caret-transparent flex flex-col mt-[-55px] min-h-[auto] min-w-[auto] outline-[3px] w-full md:mt-[-175px]"
                                    >
                                      <img
                                        src="/lp-images-files-videos-fonts/images/amala/2.png"
                                        alt=""
                                        title=""
                                        className="aspect-square box-border caret-transparent h-full max-w-full min-h-[auto] min-w-[auto] object-scale-down outline-[3px] w-[225px] md:h-auto"
                                      />
                                    </div>
                                  </div>
                                  <div
                                    className="items-center box-border caret-transparent flex justify-center min-w-[auto] outline-[3px]"
                                  >
                                    <div
                                      className="text-zinc-700 text-[12.4583px] font-semibold box-border caret-transparent leading-[14.95px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full mt-2.5 md:text-2xl md:leading-[28.8px]"
                                    >
                                      <div
                                        className="text-[12.4583px] box-border caret-transparent leading-[14.95px] outline-[3px] w-full md:text-2xl md:leading-[28.8px]"
                                      >
                                        <p
                                          className="text-gray-800 text-[12.4583px] box-border caret-transparent leading-[14.95px] outline-[3px] md:text-2xl md:leading-[28.8px]"
                                        >
                                          Standard Antioxidants
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="h-[110px] border-neutral-400/50 border-b md:h-auto"
                        middleClassName="h-[108px] md:h-[110px]"
                        middleText="Selective targeting of only cytotoxic radicals"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col justify-center min-w-[auto] outline-[3px]"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(Neutralizes beneficial radicals too)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-px md:text-sm md:leading-[21px] md:mt-[5px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[110px]"
                        middleText="Crosses blood-brain barrier for neuroprotection"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName="py-px"
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(Most cannot penetrate)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-px md:text-sm md:leading-[21px] md:mt-[5px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="h-[110px] border-neutral-400/50 border-b md:h-auto"
                        middleClassName="h-[108px] md:h-[110px]"
                        middleText="Penetrates mitochondrial membranes"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(Limited or no access)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[109px] md:h-[110px]"
                        middleText="Activates Nrf2 for endogenous antioxidant production"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full my-2.5"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/alert-triangle-yellow-512.png"
                        rightText="(Minimal or inconsistent)"
                        rightTextClassName="text-black text-[9px] box-border caret-transparent leading-[13.5px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="text-teal-800 h-[109px] md:h-[110px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleText="No pro-oxidant effect at any dose"
                        rightClassName="self-center"
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(Can become pro-oxidant)"
                        rightTextClassName="text-black text-[9px] box-border caret-transparent leading-[13.5px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[109px] md:h-[110px]"
                        middleText="Sustained energy without stimulants"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(No energy mechanism)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[109px] md:h-[110px]"
                        middleText="Enhances exercise adaptation"
                        middleTextClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(May impair adaptation)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[109px] md:h-[110px]"
                        middleText="Backed by 1,500+ peer-reviewed studies"
                        middleTextClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/alert-triangle-yellow-512.png"
                        rightText="(Mixed or limited evidence)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="border-neutral-400/50 border-b"
                        middleClassName="h-[109px] md:h-[110px]"
                        middleText="Third-party tested concentration verification"
                        middleTextClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName=""
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/alert-triangle-yellow-512.png"
                        rightText="(Often unverified)"
                        rightTextClassName="text-black text-[10px] box-border caret-transparent leading-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                  <div
                    className="items-center box-border caret-transparent flex h-[110px] justify-center min-w-[auto] outline-[3px] md:h-auto"
                  >
                    <div
                      className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                    >
                      <AmalaComparisonRow
                        rowClassName="h-[110px] md:h-auto"
                        middleClassName="mb-[-15px] pb-[15px] rounded-b-[10px] md:h-[141px] md:mb-[-25px] md:pb-10"
                        middleText="Zero side effects in clinical literature"
                        middleTextClassName="text-[14.0583px] box-border caret-transparent leading-[21.0875px] outline-[3px] md:text-xl md:leading-[30px]"
                        middleInnerClassName="text-white text-[14.0583px] box-border caret-transparent leading-[21.0875px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full md:text-xl md:leading-[30px]"
                        rightClassName="my-px py-px"
                        rightContentClassName="items-center box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
                        rightIconSrc="/lp-images-files-videos-fonts/images/amala/1.svg"
                        rightText="(Various GI and interaction issues)"
                        rightTextClassName="text-black text-[9px] box-border caret-transparent leading-[13.5px] min-h-[auto] min-w-[auto] outline-[3px] text-center mt-[5px] md:text-sm md:leading-[21px]"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        className="text-zinc-800 bg-no-repeat box-border caret-transparent flex flex-col max-w-[1170px] min-h-[auto] min-w-[auto] outline-[3px] text-start w-full bg-center md:flex-row"
      >
        <div
          className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] align-top w-full"
        >
          <div
            className="items-center box-border caret-transparent flex justify-center outline-[3px]"
          >
            <div
              className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto] outline-[3px] w-full"
            >
              <AmalaCtaBanner
                containerVariant="min-h-[auto] min-w-[auto] mt-[50px] md:mt-[70px]"
                sectionVariant="min-h-[auto]"
                wrapperVariant="min-w-[auto]"
                contentVariant="min-h-[auto] min-w-[auto]"
                buttonVariant="bg-gray-800 min-h-[auto] min-w-[auto]"
                innerButtonVariant="min-h-[auto] min-w-[auto]"
                mediaRowVariant=""
                mediaWrapperVariant="min-h-[auto] min-w-[auto]"
                mediaInnerVariant="min-h-[auto] min-w-[auto]"
                mediaItemVariant="min-h-[auto]"
                imageWrapperVariant="min-h-[auto] min-w-[auto]"
                imageSrc="/lp-images-files-videos-fonts/images/amala/20.svg"
                imageAlt=""
                imageSizes=""
                imageClassName="min-h-[auto] min-w-[auto]"
                textWrapperVariant="min-h-[auto] min-w-[auto]"
                headingTag="p"
                headingText="90-Day Money Back Guarantee"
                headingClassName="text-[16.0583px] box-border caret-transparent leading-[24.0875px] outline-[3px] md:text-base md:leading-6"
                ctaText="BUY NOW & SAVE"
                ctaTitle="BUY NOW & SAVE"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
