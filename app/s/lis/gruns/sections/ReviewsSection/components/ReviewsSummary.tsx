export const ReviewsSummary = () => {
  return (
    <div className="relative text-base box-border caret-transparent leading-[25.6px] max-w-[1200px] outline-[3px] mx-auto px-4 py-8">
      <div
        role="tablist"
        className="box-border caret-transparent flex justify-center outline-[3px] mb-6 border-b-black/10 border-b border-solid md:justify-normal"
      >
        <div
          role="tab"
          className="text-sm box-border caret-transparent leading-[22.4px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full border-green-700 px-[11.2px] py-[7px] border-b-2 border-solid md:w-[150px]"
        >
          Product reviews
        </div>
        <div
          role="tab"
          className="text-sm box-border caret-transparent leading-[22.4px] min-h-[auto] min-w-[auto] outline-[3px] text-center w-full px-[11.2px] py-[7px] border-b-transparent border-b-2 border-solid md:w-[150px]"
        >
          Store reviews
        </div>
      </div>
      <div className="relative box-border caret-transparent outline-[3px]">
        <div className="items-center box-border caret-transparent gap-x-12 flex flex-wrap outline-[3px] gap-y-4 mb-4 md:flex-nowrap">
          <div className="items-center box-border caret-transparent gap-x-4 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-4 w-full md:gap-x-8 md:w-auto">
            <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-center">
              <div className="text-5xl box-border caret-transparent leading-[48px] outline-[3px] mb-2">
                4.76
              </div>
              <div
                aria-label="4.76 stars"
                role="img"
                className="items-center box-border caret-transparent flex outline-[3px] mb-2"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                    <div className="box-border caret-transparent h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent flex h-full outline-[3px] w-[95.2476%] overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[27px] min-h-[auto] min-w-[auto] outline-[3px] w-[27px] p-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="text-sm box-border caret-transparent leading-[22.4px] outline-[3px]">
                102,551 reviews
              </div>
            </div>
            <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full md:w-[250px]">
              <div
                aria-label="5 stars, 87,689 reviews. Press to filter reviews by this rating."
                role="button"
                className="items-center box-border caret-transparent flex leading-5 outline-[3px] my-0.5"
              >
                <div className="text-sm box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-3 mr-2">
                  5
                </div>
                <div className="bg-black/10 box-border caret-transparent h-2 min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden rounded-sm">
                  <div className="bg-green-700 box-border caret-transparent h-full outline-[3px] w-[86%] rounded-l-sm">
                     
                  </div>
                </div>
              </div>
              <div
                aria-label="4 stars, 9,118 reviews. Press to filter reviews by this rating."
                role="button"
                className="items-center box-border caret-transparent flex leading-5 outline-[3px] my-0.5"
              >
                <div className="text-sm box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-3 mr-2">
                  4
                </div>
                <div className="bg-black/10 box-border caret-transparent h-2 min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden rounded-sm">
                  <div className="bg-green-700 box-border caret-transparent h-full outline-[3px] w-[9%] rounded-l-sm">
                     
                  </div>
                </div>
              </div>
              <div
                aria-label="3 stars, 3,221 reviews. Press to filter reviews by this rating."
                role="button"
                className="items-center box-border caret-transparent flex leading-5 outline-[3px] my-0.5"
              >
                <div className="text-sm box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-3 mr-2">
                  3
                </div>
                <div className="bg-black/10 box-border caret-transparent h-2 min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden rounded-sm">
                  <div className="bg-green-700 box-border caret-transparent h-full outline-[3px] w-[3%] rounded-l-sm">
                     
                  </div>
                </div>
              </div>
              <div
                aria-label="2 stars, 1,284 reviews. Press to filter reviews by this rating."
                role="button"
                className="items-center box-border caret-transparent flex leading-5 outline-[3px] my-0.5"
              >
                <div className="text-sm box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-3 mr-2">
                  2
                </div>
                <div className="bg-black/10 box-border caret-transparent h-2 min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden rounded-sm">
                  <div className="bg-green-700 box-border caret-transparent h-full outline-[3px] w-[1%] rounded-l-sm">
                     
                  </div>
                </div>
              </div>
              <div
                aria-label="1 star, 1,239 reviews. Press to filter reviews by this rating."
                role="button"
                className="items-center box-border caret-transparent flex leading-5 outline-[3px] my-0.5"
              >
                <div className="text-sm box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-3 mr-2">
                  1
                </div>
                <div className="bg-black/10 box-border caret-transparent h-2 min-h-[auto] min-w-[auto] outline-[3px] w-full overflow-hidden rounded-sm">
                  <div className="bg-green-700 box-border caret-transparent h-full outline-[3px] w-[1%] rounded-l-sm">
                     
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="items-center box-border caret-transparent gap-x-2 flex flex-wrap-reverse min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 w-full ml-auto md:flex-nowrap md:w-auto"></div>
        </div>
      </div>
      <div className="bg-black/10 box-border caret-transparent h-px outline-[3px]">
         
      </div>
      <div className="box-border caret-transparent outline-[3px] mt-4">
        <button
          aria-label="Toggle filters"
          className="text-[14.4px] font-medium items-center bg-transparent caret-transparent gap-x-2 flex justify-center tracking-[0.36px] leading-[14.4px] outline-[3px] gap-y-2 text-center w-full border px-5 py-2 rounded-[5.4px] border-black/10 md:hidden"
        >
          <span className="bg-[url('/s/lis/gruns/media/6d97ad80e3f8d85c0bde.svg')] bg-no-repeat box-border caret-transparent block h-4 min-h-[auto] min-w-[auto] outline-[3px] w-4 bg-center md:min-h-0 md:min-w-0">
             
          </span>
          Filter
        </button>
        <div className="items-center box-border caret-transparent gap-x-3 grid grid-cols-[1fr] outline-[3px] gap-y-3 mt-4 md:grid-cols-[repeat(auto-fill,minmax(190px,auto))]">
          <div className="relative items-center box-border caret-transparent flex h-[42px] leading-4 min-h-[auto] min-w-[auto] outline-[3px] w-full border overflow-hidden pl-4 pr-3 py-2.5 rounded-bl rounded-br rounded-tl rounded-tr border-solid border-black/10">
            <select
              aria-label="Sort by drop down"
              className="absolute appearance-none text-transparent [align-items:normal] bg-transparent box-content block h-full outline-[3px] text-wrap w-full z-[1] left-0 top-0"
            >
              <option
                value="mostRecent"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                Latest
              </option>
              <option
                value="oldest"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                Oldest
              </option>
              <option
                value="highestRated"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                Highest rated
              </option>
              <option
                value="lowestRated"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                Lowest rated
              </option>
              <option
                value="mostHelpful"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                Most helpful
              </option>
            </select>
            <div className="items-center caret-transparent gap-x-2 flex leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 text-ellipsis text-nowrap w-full overflow-hidden">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-nowrap overflow-hidden">
                Sort by:
                <span className="box-border caret-transparent outline-[3px] text-nowrap">
                  Latest
                </span>
              </div>
              <div className="bg-[url(data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20width=%2710%27%20height=%277%27%20viewBox=%270%200%2010%206%27%3E%3Cpath%20fill-rule=%27evenodd%27%20d=%27M.72.47a.75.75%200%200%201%201.06%200l3.47%203.47L8.72.47a.75.75%200%200%201%201.06%201.06l-4%204a.75.75%200%200%201-1.06%200l-4-4a.75.75%200%200%201%200-1.06%27%20clip-rule=%27evenodd%27/%3E%3C/svg%3E)] bg-no-repeat bg-size-[100%] box-border caret-transparent h-1.5 min-h-[auto] min-w-2.5 outline-[3px] text-nowrap w-2.5 bg-center ml-auto">
                 
              </div>
            </div>
          </div>
          <div className="relative items-center box-border caret-transparent flex h-[42px] leading-4 min-h-[auto] min-w-[auto] outline-[3px] w-full border overflow-hidden pl-4 pr-3 py-2.5 rounded-bl rounded-br rounded-tl rounded-tr border-solid border-black/10">
            <select
              aria-label="Rating drop down"
              className="absolute appearance-none text-transparent [align-items:normal] bg-transparent box-content block h-full outline-[3px] text-wrap w-full z-[1] left-0 top-0"
            >
              <option
                value="all"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                All
              </option>
              <option
                value="1"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                1 star
              </option>
              <option
                value="2"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                2 stars
              </option>
              <option
                value="3"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                3 stars
              </option>
              <option
                value="4"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                4 stars
              </option>
              <option
                value="5"
                className="text-black items-center box-border caret-transparent gap-x-2 min-h-6 min-w-6 outline-[3px] gap-y-2 pb-0 px-0"
              >
                5 stars
              </option>
            </select>
            <div className="items-center caret-transparent gap-x-2 flex leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 text-ellipsis text-nowrap w-full overflow-hidden">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] text-nowrap overflow-hidden">
                Rating
              </div>
              <div className="bg-[url(data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20width=%2710%27%20height=%277%27%20viewBox=%270%200%2010%206%27%3E%3Cpath%20fill-rule=%27evenodd%27%20d=%27M.72.47a.75.75%200%200%201%201.06%200l3.47%203.47L8.72.47a.75.75%200%200%201%201.06%201.06l-4%204a.75.75%200%200%201-1.06%200l-4-4a.75.75%200%200%201%200-1.06%27%20clip-rule=%27evenodd%27/%3E%3C/svg%3E)] bg-no-repeat bg-size-[100%] box-border caret-transparent h-1.5 min-h-[auto] min-w-2.5 outline-[3px] text-nowrap w-2.5 bg-center ml-auto">
                 
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div
              role="checkbox"
              aria-label="Toggle photos & videos filter"
              className="box-border caret-transparent outline-[3px]"
            >
              <label className="relative items-center box-border caret-transparent flex tracking-[-0.48px] leading-6 outline-[3px] text-nowrap before:accent-auto before:bg-white before:box-border before:caret-transparent before:text-black before:block before:text-base before:not-italic before:normal-nums before:font-normal before:h-5 before:tracking-[-0.48px] before:leading-6 before:list-outside before:list-disc before:min-h-[auto] before:min-w-[auto] before:outline-[3px] before:pointer-events-auto before:text-start before:no-underline before:indent-[0px] before:normal-case before:text-nowrap before:visible before:w-5 before:border before:mr-2 before:rounded-[3px] before:border-separate before:border-solid before:border-black/10 before:font-work_sans">
                Photos &amp; videos
              </label>
            </div>
          </div>
        </div>
      </div>
      <div className="relative box-border caret-transparent leading-6 outline-[3px] pt-8">
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Sabrina M avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  SM
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    SabrinaM
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  2 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Taste yummy
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    Been taking for about a week and so far I’m really happy!
                    Smell is harsh but the taste is good and been keeping me
                    regular !!!
                  </span>
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-wrap outline-[3px] gap-y-4 mt-4">
              <div
                role="button"
                aria-label="Preview image for Taste yummy"
                className="relative box-border caret-transparent h-16 min-h-[auto] min-w-[auto] outline-[3px] w-16 md:h-24 md:w-24"
              >
                <img
                  src="/s/lis/gruns/media/3eipkmhbiaj24zspqwlt62chb007.jpg"
                  alt="Preview image for Taste yummy"
                  className="box-border caret-transparent h-full max-w-full object-cover outline-[3px] w-full"
                />
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Leann K avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  LK
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    LeannK
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  4 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Tastes good!
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    I love the taste and consistency of it! Easy to eat and have
                    noticed an improvement in my overall health.
                  </span>
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-wrap outline-[3px] gap-y-4 mt-4">
              <div
                role="button"
                aria-label="Preview image for Tastes good!"
                className="relative box-border caret-transparent h-16 min-h-[auto] min-w-[auto] outline-[3px] w-16 md:h-24 md:w-24"
              >
                <img
                  src="/s/lis/gruns/media/rvq1wrg7bd8w9j6azndi67vaejub.jpg"
                  alt="Preview image for Tastes good!"
                  className="box-border caret-transparent h-full max-w-full object-cover outline-[3px] w-full"
                />
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns-olipop-kids"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns Kids OLIPOP® - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Nicole A avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  NA
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    NicoleA
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  8 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  5 stars
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    5 stars
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Kara H avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  KH
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    KaraH
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  17 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Five Stars
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    I haven’t missed a day since I started! They taste great,
                    and I feel healthier.
                  </span>
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-wrap outline-[3px] gap-y-4 mt-4">
              <div
                role="button"
                aria-label="Preview image for Five Stars"
                className="relative box-border caret-transparent h-16 min-h-[auto] min-w-[auto] outline-[3px] w-16 md:h-24 md:w-24"
              >
                <img
                  src="/s/lis/gruns/media/66nk5glqrtyks9r9g8kzbw5xvsqq.jpg"
                  alt="Preview image for Five Stars"
                  className="box-border caret-transparent h-full max-w-full object-cover outline-[3px] w-full"
                />
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Susan j avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  SJ
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    Susanj
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  32 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Sugar free olipops review ease of taking
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    The flavor of these is wonderful. My husband is now taking
                    these also. We really feel our gut health has improved along
                    with really liking the ease of taking them.
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns-olipop"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns OLIPOP® - Sugar-Free / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Amber R avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  AR
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    AmberR
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  34 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Gives, me energy, helps...
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    Gives, me energy, helps my digestive track. I don&#39;t even
                    take other supplements
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Amy S avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  AS
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    AmyS
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  38 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Lioved the Fancy Flavor!
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    It&#39;s such a fun little healthy treat. I use it as
                    dessert at night.
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns-olipop"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns OLIPOP® - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Rebecca E avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  RE
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    RebeccaE
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  48 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Love the taste! ...
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    Love the taste! Wish these were always available.
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns-olipop"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns OLIPOP® - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Cynthia S avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  CS
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    CynthiaS
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  55 minutes ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Great product
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    They are so easy to add to your routine. They taste great
                    and I’ve used them for the sweet after lunch. My doctor even
                    told me that my labs were better and the vitamins probably
                    helped me. Keep up a good product.
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px] mb-8">
           
        </div>
        <div className="box-border caret-transparent gap-x-4 flex flex-col outline-[3px] gap-y-4 pb-8 md:flex-row">
          <div className="box-border caret-transparent max-w-full min-h-[auto] min-w-[220px] outline-[3px] md:max-w-[220px]">
            <div className="items-center box-border caret-transparent flex outline-[3px]">
              <div
                role="img"
                aria-label="Cody G avatar"
                className="items-center box-border caret-transparent flex h-[46px] min-h-[auto] min-w-[46px] outline-[3px] w-[46px] border mr-2 p-[3px] rounded-[50%] border-solid border-black/10"
              >
                <div className="text-sm font-bold items-center bg-gray-200 box-border caret-transparent flex h-full justify-center leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] w-full rounded-[50%] font-arial">
                  CG
                </div>
              </div>
              <div className="box-border caret-transparent leading-[19.2px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center box-border caret-transparent gap-x-2 flex outline-[3px] gap-y-2">
                  <div className="box-border caret-transparent max-w-[149.28px] min-h-[auto] min-w-[auto] outline-[3px] text-ellipsis text-nowrap overflow-hidden">
                    CodyG
                  </div>
                  <div
                    role="img"
                    aria-label="This customer received a perk in exchange for their honest review."
                    className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]"
                  >
                    <div className="box-border caret-transparent outline-[3px]">
                      <img
                        src="/s/lis/gruns/media/icon-27.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3 outline-[3px] w-3"
                      />
                    </div>
                  </div>
                </div>
                <div
                  role="img"
                  aria-label="Purchase verified by Junip"
                  className="box-border caret-transparent inline-block outline-[3px]"
                >
                  <div className="text-sm items-center box-border caret-transparent flex leading-[14px] outline-[3px] mt-1">
                    <div className="items-start box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                      <img
                        src="/s/lis/gruns/media/icon-28.svg"
                        alt="Icon"
                        className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                      />
                    </div>
                    <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] ml-1">
                      Verified buyer
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full">
            <div className="items-center box-border caret-transparent flex justify-between outline-[3px] mb-4">
              <div
                aria-label="5 stars"
                role="img"
                className="items-center box-border caret-transparent flex ml-[-3px] min-h-[auto] min-w-[auto] outline-[3px]"
              >
                <div className="relative items-center box-border caret-transparent flex min-h-[auto] min-w-[auto] outline-[3px]">
                  <div className="items-center box-border caret-transparent gap-x-1 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-1">
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-25.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                  <div className="absolute items-center box-border caret-transparent gap-x-1 flex h-full outline-[3px] gap-y-1 w-full overflow-hidden left-0 top-0">
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                    <div className="box-border caret-transparent shrink-0 h-[18px] min-h-[auto] min-w-[auto] outline-[3px] w-[18px]">
                      <img
                        src="/s/lis/gruns/media/icon-26.svg"
                        alt="Icon"
                        className="box-border caret-transparent outline-[3px] m-auto"
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="items-center box-border caret-transparent gap-x-2 flex min-h-[auto] min-w-[auto] outline-[3px] gap-y-2">
                <div className="text-sm box-border caret-transparent leading-[21px] min-h-[auto] min-w-[auto] opacity-80 outline-[3px] text-nowrap">
                  1 hour ago
                </div>
              </div>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex flex-col justify-between outline-[3px] gap-y-4 md:flex-row">
              <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                <div
                  role="heading"
                  className="text-lg font-semibold box-border caret-transparent leading-[27px] outline-[3px] mb-2"
                >
                  Like Gruns!!
                </div>
                <div className="box-border caret-transparent outline-[3px]">
                  <span className="box-border caret-transparent outline-[3px]">
                    Been taking gruns for awhile and like them!! It was great
                    came out with a new flavor!!
                  </span>
                </div>
              </div>
            </div>
            <div className="text-sm box-border caret-transparent leading-[21px] outline-[3px] mt-4">
              Review for
              <a
                href="https://gruns.co/products/gruns-olipop"
                className="box-border caret-transparent outline-[3px]"
              >
                Grüns OLIPOP® - Low Sugar / 28 Packs
              </a>
            </div>
            <div className="box-border caret-transparent gap-x-4 flex outline-[3px] gap-y-4 mt-6">
              <div className="text-sm items-center box-border caret-transparent flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px]">
                <div className="items-center bg-[url('https://widgets.juniphq.com/v1/images/4402299c7b701bcfc128.svg')] bg-no-repeat bg-size-[100%] box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3 bg-center mr-1.5">
                   
                </div>
                <div className="box-border caret-transparent min-h-[auto] min-w-[auto] outline-[3px]">
                  Would recommend
                </div>
              </div>
              <div className="text-sm box-border caret-transparent gap-x-2 flex leading-[14px] min-h-[auto] min-w-[auto] outline-[3px] gap-y-2 ml-auto">
                <div className="box-border caret-transparent hidden min-h-0 min-w-0 outline-[3px] md:block md:min-h-[auto] md:min-w-[auto]">
                  Helpful
                </div>
                <div
                  aria-label="Up vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-29.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
                <div className="bg-black/10 box-border caret-transparent h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-px">
                   
                </div>
                <div
                  aria-label="Down vote review action - 0"
                  role="button"
                  className="box-border caret-transparent gap-x-1 flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] gap-y-1"
                >
                  <div className="items-end box-border caret-transparent flex h-3.5 min-h-[auto] min-w-[auto] outline-[3px] w-3.5">
                    <img
                      src="/s/lis/gruns/media/icon-30.svg"
                      alt="Icon"
                      className="box-border caret-transparent h-3.5 outline-[3px] w-3.5"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-black/10 box-border caret-transparent h-px outline-[3px]">
           
        </div>
      </div>
      <button className="text-white text-[14.4px] font-medium bg-green-700 caret-transparent block tracking-[0.36px] leading-[14.4px] outline-[3px] text-center border border-green-700 mt-4 mx-auto px-5 py-3 rounded-[5.4px]">
        See more reviews
      </button>
      <div className="text-sm items-center box-border caret-transparent gap-x-1 flex grayscale-[1] h-[17px] justify-center leading-[14px] outline-[3px] gap-y-1 text-nowrap mt-4">
        Reviewed on
        <a
          href="https://junip.co/reviews/gruns?utm_source=powered_by_footer&referrer=https%3A%2F%2Fgruns.co%2Fpages%2Ffiber"
          aria-label="See Grüns on Junip (opens in a new tab)"
          className="bg-[url('https://widgets.juniphq.com/v1/images/58672aa73f33fbf4a708.svg')] bg-no-repeat bg-size-[100%] caret-transparent block h-[15px] min-h-[auto] min-w-[auto] outline-[3px] text-nowrap w-10 bg-bottom pt-0.5"
        >
           
        </a>
      </div>
    </div>
  );
};
