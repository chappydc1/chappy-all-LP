export const HearBenefits = () => {
  return (
    <section className="caret-transparent outline-[3px] text-center py-[25px]">
      <div className="caret-transparent flex outline-[3px]">
        <div className="border-r-stone-300 caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-[25px] border-r md:px-5">
          <img
            src="/lp-images-files-videos-fonts/images/hear/45-day-trial.svg"
            alt=""
            className="caret-transparent outline-[3px] align-baseline w-[30px] mb-2.5 mx-auto md:w-10 md:mb-[15px]"
          />
          <p className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]">
            45-Day
            <br className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]" />
            No-Risk Trial
          </p>
        </div>
        <div className="border-r-stone-300 caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-[25px] border-r md:px-5">
          <img
            src="/lp-images-files-videos-fonts/images/hear/money-back.svg"
            alt=""
            className="caret-transparent outline-[3px] align-baseline w-[30px] mb-2.5 mx-auto md:w-10 md:mb-[15px]"
          />
          <p className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]">
            Money-Back
            <br className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]" />
            Guarantee
          </p>
        </div>
        <div className="caret-transparent min-h-[auto] min-w-[auto] outline-[3px] w-full px-2.5 py-[25px] md:px-5">
          <img
            src="/lp-images-files-videos-fonts/images/hear/warranty.svg"
            alt=""
            className="caret-transparent outline-[3px] align-baseline w-[30px] mb-2.5 mx-auto md:w-10 md:mb-[15px]"
          />
          <p className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]">
            3 Year
            <br className="text-base caret-transparent leading-5 outline-[3px] md:text-[19px] md:leading-[23.75px]" />
            Warranty
          </p>
        </div>
      </div>
    </section>
  );
};
